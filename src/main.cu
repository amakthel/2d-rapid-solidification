...
void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine,
  ...
  if constexpr (param::if_load) {
    ...
    // sim::num_axes*sim::num_orientation*sizeof(real), cudaMemcpyHostToDevice);
    ...
    //     set_orientation(h_orientation+sim::num_axes, alpha1, beta1, gamma1);
  ...
  void set_fields_output(
      real * h_c, real * h_phi, real * h_psi, real * h_T, real * h_T_fine,
      constants * h_para, real * h_orientation, real * d_c, real * d_phi,
      real * d_psi, real * d_T, real * d_T_fine, constants * d_para,
      size_t size_grid, size_t size_grid_T, real start_time, long int step,
      dim3 numBlocks, dim3 threadsPerBlock) {
  ...
  void set_check_point(real * h_c, real * h_phi, real * h_psi, real * h_T,
                       real * d_c, real * d_phi, real * d_psi, real * d_T,
                       constants * h_para, size_t size_grid, size_t size_grid_T,
                       long int step, int final_step) {
  ...
  void set_pull_back(real * h_c, real * h_phi, real * h_psi, real * h_T,
    ...
    snprintf(offset_name, sizeof(offset_name), "%s/data/offset.txt",
             path_input);
                  ...
                  constants * para, int seed) // flag
  {
    ...
    if (sim::num_orientation == 2)
      set_orientation(orientation + sim::num_axes, alpha1, beta1, gamma1);
...
#define r_pool (Nx - 200) // radius of the circular melt pool
  void set_nuclei_melt_pool(real * c, real * phi, real * psi,
                            real * orientation, constants * para, int seed) {
      ...
      for (i = 0; i < Nx; i++) {
        ...
        if (pow2(i - Nx) + pow2(j - Ny) > pow2(r_pool - initial_max_radius)) {
          ...
          for (l = 0; l < num_nuclei; l++) {
            dis =
                sqrt(pow2(i - nuclei[3 * l + 0]) + pow2(j - nuclei[3 * l + 1]));
            dis_jlow =
  ...
  __device__ real get_anisotropy(real * phi, real * orientation, real * as,
                                 real * ak, int i, int j, int a) {
    int psa = psN(i, j, a);
    real phi00 = phi[psa];
    ...
    if (abs_gradient_phi > 1.0e-15) {
      dtheta_dx = (dphi_dx * dphi_dx_dy - dphi_dy * dphi_dx_dx) /
                  pow2(abs_gradient_phi);
      dtheta_dy = (dphi_dx * dphi_dy_dy - dphi_dy * dphi_dx_dy) /
                  pow2(abs_gradient_phi);
    ...
    dnp4_dtheta_2 +=
        3.0 * pow2(nyp * dnyp_dtheta_1) + pow3(nyp) * dnyp_dtheta_2;
    dnp4_dtheta_2 +=
        3.0 * pow2(nzp * dnzp_dtheta_1) + pow3(nzp) * dnzp_dtheta_2;
    dnp4_dtheta_2 *= 4;
    ...
    real anisotropy = pow2(*as) * laplacian_phi +
                      2.0 * (*as) * da_dtheta_1 *
                          (dtheta_dx * dphi_dx + dtheta_dy * dphi_dy) +
...
/// add noise
  ...
  __device__ __forceinline__ void compute_boundary_individual(
      real * data, int i, int j, int a) {
    if (i == imin)
                            ...
                            long int step) // flag
  {
    ...
    if (i > 0 && i < Nx - 1 && j > 0 && j < Ny - 1) {
      ...
      real src10 = (F10 + F01 + Fm0 + F0m) /
                   (dx * dx); // the divergence evaluated by <10>
  ...
  __global__ void compute_c_CALPHAD(
      real * c, real * phi, real * psi, real * T_fine, real * c_next,
      real * phi_next,
      real * psi_next, // flag
                                ...
                                long int step) // flag
  {
    ...
    int iTn = (i_left_end + Nx - 3) /
              ratio_dxT_dx; // the last iT index for the right border of the
  ...
  __global__ void interpolate_temperature(real * T, real * T_fine,
                                          constants * para, int i_left_end,
                                          long int step) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
  ...
  __host__ __device__ real get_delta(constants * para, int i,
                                     int i_offset_history,
...
#define L0_Liq(T) (5144.29)
  ...
  {
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "%s/data/%s_%d.txt", path_input,
             field, index);
  ...
  void load_orientation(real * orientation) {
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "%s/data/orientation.txt",
             path_input);
  ...
  void save_parameters(constants * para, int i_tip) {
    ...
    fprintf(fp,
            "%-13g l_offset_Vp, unit: [W]; offset due to pulling velocity\n",
            para->l_offset_Vp);
    ...
    fprintf(
        ...
        eps2 * 1.0);
    fprintf(fp,
            "%-13g epk1, anisotropy parameter of the kinetic coefficient \n",
            epk1);
...
#define LENMAX 256
  ...
  void DisplayDeviceProperties(int index) {
    ...
    if (cudaSuccess == cudaGetDeviceProperties(&deviceProp, index)) {
      ...
      printf("\nNumber of registers per block \t\t %d",
...
#define num_data (num_y_data)
  ...
  void save_tip(long int step, constants *h_para, real delta, real T_tip,
                TIP tip, real V_tip, real *h_c, real *h_phi) // flag
  {
...
#else
      real delta = get_delta(h_para, tip.i, h_para->i_offset_history,
                             h_para->l_offset_Vp);
