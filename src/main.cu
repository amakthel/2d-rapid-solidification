...
namespace mat = Parameters::Material;
namespace param = Parameters;
namespace sim = Parameters::Simulation;

constexpr auto pos(std::size_t x, std::size_t y) -> std::size_t {
  return y * sim::Nx + x;
}
constexpr auto psN(std::size_t x, std::size_t y, std::size_t a) -> std::size_t {
  return a * sim::Nx * sim::Ny + y * sim::Nx + x; // position in the phi matrix
}
constexpr auto psT(std::size_t x, std::size_t y) -> std::size_t {
  return y * sim::NxT + x;
}

constexpr auto pow2(real x) -> real { return x * x; }
constexpr auto pow2(std::size_t x) -> std::size_t { return x * x; }
constexpr auto pow3(real x) -> real { return x * x * x; }
constexpr auto pow4(real x) -> real { return pow2(pow2(x)); }
constexpr auto pow5(real x) -> real { return x * x * x * x * x; }
constexpr auto pow6(real x) -> real { return pow2(pow3(x)); }
constexpr auto pow7(real x) -> real { return x * x * x * x * x * x * x; }
constexpr auto pow8(real x) -> real { return pow2(pow4(x)); }
constexpr auto pow9(real x) -> real { return pow3(pow3(x)); }

constexpr auto qq(real p) -> real {
  return (0.5 * sim::AA * (1.0 - p) - 0.25 * (sim::AA - 1) * pow2(1.0 - p));
}
constexpr auto dqq_dp(real p) -> real {
  return -0.5 * (1.0 + p * (sim::AA - 1)); // first dirivative
}

constexpr auto gg(real p) -> real {
  return 1.875 * p - 1.25 * pow3(p) + 0.375 * pow5(p); // 15.0/8.0 = 1.875
}

constexpr auto dgg_dp(real p) -> real {
  return 1.875 * pow2(1.0 - pow2(p)); // first dirivative
}
constexpr auto dgg_dpdp(real p) -> real {

  return 7.5 * (p) * (pow2(p) - 1.0); // second dirivative
}

int main(int argc, char **argv) {
  // set the index of device and display the device information
  if (set_cuda_device() != 0)
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////
  /// matrix definitions
  size_t size_grid = sim::Nx * sim::Ny * sizeof(real);
  real *h_c, *h_phi, *h_psi,
      *h_orientation; // psi: sim::num_orientation - 1 + Σ phi
  ...
  h_phi = (real *)malloc(sim::num_orientation * size_grid);
  h_psi = (real *)malloc(size_grid);
  h_orientation =
      (real *)malloc(sim::num_axes * sim::num_orientation * sizeof(real));

  cudaMalloc((void **)&d_c, size_grid);
  cudaMalloc((void **)&d_phi, sim::num_orientation * size_grid);
  cudaMalloc((void **)&d_psi, size_grid);
  cudaMalloc((void **)&d_c_next, size_grid);
  cudaMalloc((void **)&d_phi_next, sim::num_orientation * size_grid);
  cudaMalloc((void **)&d_psi_next, size_grid);
  cudaMalloc((void **)&d_orientation,
             sim::num_axes * sim::num_orientation * sizeof(real));
  cudaMalloc((void **)&d_state, sim::Nx * sim::Ny * sizeof(curandState));

  dim3 threadsPerBlock(sim::BLOCK_SIZE_X, sim::BLOCK_SIZE_Y);
  dim3 numBlocks(sim::Nx / threadsPerBlock.x, sim::Ny / threadsPerBlock.y);
  ...
  threadsPerBlockT = dim3(sim::BLOCK_SIZE_T_X, sim::BLOCK_SIZE_T_Y);
  ...
  for (step = h_para->initial_step; step <= h_para->total_step; step++) {
    ...
    compute_psi<<<numBlocks, threadsPerBlock>>>(
        ...
        d_state, d_para, step);
    if constexpr (param::simModel == param::Model::CALPHAD) {
    ...
    h_para->l_offset_Vp += Vp_ramp(step * mat::tau0 * h_para->dt) * h_para->dt *
                           mat::tau0 / (SS * mat::W0);
    //////////////////////////////////////////////////////////////////////////////////////////////////
...
void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine,
  ...
  if constexpr (param::if_load) {
    ...
    // sim::num_orientation*size_grid, cudaMemcpyHostToDevice);
    ...
    for (std::size_t j = 0; j < sim::Ny; j++) {
      for (std::size_t i = 0; i < sim::Nx; i++) {
        h_psi[pos(i, j)] = get_local_psi(h_phi, i, j);
      }
    }

    // set_orientation(h_orientation, alpha0, beta0, gamma0);
    // if (sim::num_orientation==2)
    //     set_orientation(h_orientation+sim::num_axes, alpha1, beta1,
    //     gamma1);

    save_orientation(h_orientation);
    cudaMemcpy(d_c, h_c, size_grid, cudaMemcpyHostToDevice);
    cudaMemcpy(d_phi, h_phi, sim::num_orientation * size_grid,
               cudaMemcpyHostToDevice);
    cudaMemcpy(d_psi, h_psi, size_grid, cudaMemcpyHostToDevice);
    cudaMemcpy(d_orientation, h_orientation,
               sim::num_axes * sim::num_orientation * sizeof(real),
    ...
    save_parameters(h_para, sim::i_tip_target);
  }
}
...
void set_fields_output(real *h_c, real *h_phi, real *h_psi, real *h_T,
  ...
  if (step % h_para->step_fields == 0 || step == h_para->initial_step) // flag
  {
    ...
    cudaMemcpy(h_phi, d_phi, sim::num_orientation * size_grid,
               cudaMemcpyDeviceToHost);
    ...
    printf("%-10ld ", step);
    printf("%-10.3f ", step * h_para->dt * mat::tau0 / 1000);
                                 ...
                                 int seed) // for polycrystalline
{
  ...
  for (std::size_t a = 0; a < sim::num_orientation; a++) {
    ...
    orientation[a * sim::num_axes + 0] = x1;
    orientation[a * sim::num_axes + 1] = x2;
    orientation[a * sim::num_axes + 2] = x3;
    orientation[a * sim::num_axes + 3] = y1;
    orientation[a * sim::num_axes + 4] = y2;
    orientation[a * sim::num_axes + 5] = y3;
    orientation[a * sim::num_axes + 6] = z1;
    orientation[a * sim::num_axes + 7] = z2;
    orientation[a * sim::num_axes + 8] = z3;
  }
}
...
{
  int i, j, l, a, nuclei[sim::num_nuclei_target * 3];
  // generate_random_orientation(orientation, seed);
  set_orientation(orientation, sim::alpha0, sim::beta0, sim::gamma0);
  if (sim::num_orientation == 2)
    set_orientation(orientation + sim::num_axes, sim::alpha1, sim::beta1,
                    sim::gamma1);

  /////////////////////////////////// set the distribution of grains
  srand(seed);
  for (i = 0; i < sim::num_nuclei_target; i++) {
    int random = (real)rand();
    int random2 = random % (sim::Ny * sim::initial_nuclei_border_i);
    nuclei[3 * i + 0] = random2 % sim::initial_nuclei_border_i; // i
    nuclei[3 * i + 1] = random2 / sim::initial_nuclei_border_i; // j
    random = rand();
    nuclei[3 * i + 2] = random % sim::num_orientation;

    if (nuclei[3 * i + 2] == 0 && nuclei[3 * i + 1] >= sim::j_border) {
      nuclei[3 * i + 1] = int((nuclei[3 * i + 1] - sim::j_border) *
                              sim::j_border / (sim::Ny - sim::j_border));
    }
    if (nuclei[3 * i + 2] == 1 && nuclei[3 * i + 1] < sim::j_border) {
      nuclei[3 * i + 1] =
          int(nuclei[3 * i + 1] * (sim::Ny - sim::j_border) / sim::j_border) +
          sim::j_border;
    }
  }
  ...
  for (j = 0; j < sim::Ny; j++) {
    for (i = 0; i < sim::Nx; i++) {
      /////////////////////// liquid
      c[pos(i, j)] = 1.0;
      for (a = 0; a < sim::num_orientation; a++)
        phi[psN(i, j, a)] = -1.0;

      /////////////////////// find the nearest nucleus
      min_dis = 100000.0;
      for (l = 0; l < sim::num_nuclei_target; l++) {
        dis = sqrt(pow2(real(i) - nuclei[3 * l + 0]) +
                   pow2(real(j) - nuclei[3 * l + 1]));
        dis_jlow = sqrt(
            pow2(real(i) - nuclei[3 * l + 0]) +
            pow2(real(j) - (nuclei[3 * l + 1] -
                            real(sim::Ny - 2)))); // nuclei on the upper mirror
        dis_jup = sqrt(
            pow2(real(i) - nuclei[3 * l + 0]) +
            pow2(real(j) - (nuclei[3 * l + 1] +
                            real(sim::Ny - 2)))); // nuclei on the lower mirror
      ...
      if (min_dis < sim::initial_max_radius) {
        c[pos(i, j)] = mat::ke * abs(para->delta_initial);
...
void set_nuclei_melt_pool(real *c, real *phi, real *psi, real *orientation,
  ...
  int l, a, nuclei[sim::num_nuclei_target * 2 * 3];
  generate_random_orientation(orientation, seed);
  ...
  real random, possibility,
      num_lattice_outside_ellipse =
          sim::Nx * sim::Ny - pi * pow2(sim::Nx - 200) / 4.0;

  for (std::size_t j{}; j < sim::Ny; j++) {
    for (std::size_t i{}; i < sim::Nx; i++) {
      if (pow2(i - sim::Nx) + pow2(j - sim::Ny) > pow2(sim::Nx - 200)) {
        random = (real)rand() / (real)RAND_MAX;
        possibility = sim::num_nuclei_target / (num_lattice_outside_ellipse);
        if (random < possibility) {
          nuclei[3 * ind + 0] = i;
          nuclei[3 * ind + 1] = j;
          nuclei[3 * ind + 2] =
              (int)floor(random / (possibility / sim::num_orientation));
          ind++;
        }
      }
    }
  ...
  for (std::size_t j{}; j < sim::Ny; j++) {
    for (std::size_t i{}; i < sim::Nx; i++) {
      ...
      if (pow2(i - sim::Nx) + pow2(j - sim::Ny) >
          pow2(sim::Nx - 200 - sim::initial_max_radius)) {
        /////////////////////// find the nearest nuclei
        min_dis = 100000.0;
        for (l = 0; l < num_nuclei; l++) {
          dis = sqrt(pow2(i - nuclei[3 * l + 0]) + pow2(j - nuclei[3 * l + 1]));
          dis_jlow =
              sqrt(pow2(i - nuclei[3 * l + 0]) +
                   pow2(j - (nuclei[3 * l + 1] -
                             (sim::Ny - 2)))); // nucleus on the upper mirror
          dis_jup =
              sqrt(pow2(i - nuclei[3 * l + 0]) +
                   pow2(j - (nuclei[3 * l + 1] +
                             (sim::Ny - 2)))); // nucleus on the lower mirror
                        ...
                        constants *para) // flag
{
  ...
  real itip = sim::i_tip_target + 0.5 * sin(2.0 * pi * j / sim::Ny);

  // real pp = -tanh( (i-itip)*dx/sqrt(2.0) ); // consider going back for
  // this part ... phi[ps] = pp; real cc = exp(bb*(1.0 + gg(pp))); //
  // 0.3184*tanh(
  // ((i-itip)*dx-0.08)/0.67 ) + 0.6816;

  phi[ps] = -tanh((i - itip) * sim::dx / sqrt(2.0));
  real cc = 0.3184 * tanh(((i - itip) * sim::dx - 0.08) / 0.67) + 0.6816;
...
__global__ void initial_temperature(real *T, constants *para) {
  ...
  T[psT(iT, jT)] = para->Tcold + (iT - 1) * SS * mat::W0 * sim::dx *
                                     sim::ratio_dxT_dx * mat::G;
}
...
__device__ real get_anisotropy(real *phi, real *orientation, real *as, real *ak,
  ...
  real dphi_dx = (phi10 - phim0) / (2.0 * sim::dx);
  real dphi_dy = (phi01 - phi0m) / (2.0 * sim::dx);
  real abs_gradient_phi = sqrt(pow2(dphi_dx) + pow2(dphi_dy));

  real dphi_dx_dx = (phi10 - 2.0 * phi00 + phim0) / (sim::dx * sim::dx);
  real dphi_dy_dy = (phi01 - 2.0 * phi00 + phi0m) / (sim::dx * sim::dx);
  real dphi_dx_dy = (phi11 - phim1 - phi1m + phimm) / (4. * sim::dx * sim::dx);
  ...
  real x1 = orientation[a * sim::num_axes + 0];
  real x2 = orientation[a * sim::num_axes + 1];
  real y1 = orientation[a * sim::num_axes + 3];
  real y2 = orientation[a * sim::num_axes + 4];
  real z1 = orientation[a * sim::num_axes + 6];
  real z2 = orientation[a * sim::num_axes + 7];
  ...
  real da_dtheta_1{};
  real da_dtheta_2{};
  if constexpr (param::if_1_eps) {
    *ak = 1.0 - 3.0 * mat::epk1 + 4.0 * mat::epk1 * np4;
    *as = 1.0 - 3.0 * mat::eps1 + 4.0 * mat::eps1 * np4;
    da_dtheta_1 = 4.0 * mat::eps1 * dnp4_dtheta_1;
    da_dtheta_2 = 4.0 * mat::eps1 * dnp4_dtheta_2;
  } else {
    *ak = 1.0 + mat::epk1 * (np4 - 0.6);
    *as = 1.0 + mat::eps1 * (np4 - 0.6) +
          mat::eps2 * (3.0 * np4 + 66.0 * pow2(nxp * nyp * nzp) - 17.0 / 7.0);
    ...
    da_dtheta_1 = (mat::eps1 + 3.0 * mat::eps2) * dnp4_dtheta_1 +
                  66.0 * mat::eps2 * dnp2_dtheta_1;
    da_dtheta_2 = (mat::eps1 + 3.0 * mat::eps2) * dnp4_dtheta_2 +
                  66.0 * mat::eps2 * dnp2_dtheta_2;
  }
...
// V100 has 256^2 registers per block; a real needs 2 registers; a int needs
// 1 register
__global__ void compute_phi(real *c, real *phi, real *psi, real *T_fine,
  ...
  if (i > 0 && i < sim::Nx - 1 && j > 0 && j < sim::Ny - 1) {
    for (a = 0; a < sim::num_orientation; a++) {
      int psa = psN(i, j, a);
      real phi00 = phi[psa];

      real sum_phi = 0;
      for (int b = 0; b < sim::num_orientation; b++)
        if (b != a)
...
#if (ifTFC)
      real T00 = T_fine[ps];
      delta = (T00 - mat::Tmelt) / mat::deltaT0;
#else
      delta = get_delta(para, i, i_offset_history, l_offset_Vp);
      omega = -6.0 * lambda *
              (2 * ke +
               (ke - 1.0) / bb * delta); // coupling between neighboring grains
      real T00 = delta * deltaT0 + Tmelt;
#endif

      if constexpr (param::simModel == Parameters::Model::A) {
        term += -mat::lambda * dgg_dp(phi00) *
                (c00 + delta * exp(mat::bb * (1.0 + gg(phi00)))); //
        term += -omega * 0.5 * (1.0 + phi00) *
                sum_phi; // coupling between neighboring grains
      } else if constexpr (param::simModel == Parameters::Model::B) {
        term += -mat::lambda * dgg_dp(phi00) *
                (c00 + delta * (mat::ke - 1.0) / (2.0 * mat::bb)); //
        term += -omega * 0.5 * (1.0 + phi00) *
                sum_phi; // coupling between neighboring grains
      } else if constexpr (param::simModel == Parameters::Model::CALPHAD) {
        term += (Gl(T00, c00) - Gs(T00, c00)) * dgg_dp(phi00) /
                (2.0 * mat::hh * mat::v0); //
      }
...
__device__ __forceinline__ void compute_boundary_individual(real *data, int i,
                                                            int j, int a) {
  if (i == sim::imin)
    data[psN(0, j, a)] = data[psN(sim::imin, j, a)];
  else if (i == sim::imax)
    data[psN(sim::Nx - 1, j, a)] = data[psN(sim::imax, j, a)];
  if (j == sim::jmin)
    data[psN(i, 0, a)] = data[psN(i, sim::jmin, a)];
  else if (j == sim::jmax)
    data[psN(i, sim::Ny - 1, a)] = data[psN(i, sim::jmax, a)];
}
...
__global__ void compute_psi(real *c, real *phi, real *psi, real *c_next,
  ...
  if (i > 0 && i < sim::Nx - 1 && j > 0 && j < sim::Ny - 1) {
    ...
    real psi_temp = sim::num_orientation - 1;
    if (psi_next[ps] > 1.0) {
      psi_temp = sim::num_orientation - 1;
      for (a = 0; a < sim::num_orientation; a++) {
                          ...
                          long int step) // flag
{
  ...
  if (i > 0 && i < sim::Nx - 1 && j > 0 && j < sim::Ny - 1) {
    std::size_t ps00 = pos(i, j);
    std::size_t ps10 = pos(i + 1, j);
    std::size_t psm0 = pos(i - 1, j);
    std::size_t ps01 = pos(i, j + 1);
    std::size_t ps0m = pos(i, j - 1);
    std::size_t ps11 = pos(i + 1, j + 1);
    std::size_t psm1 = pos(i - 1, j + 1);
    std::size_t ps1m = pos(i + 1, j - 1);
    ...
    real F10 = 0.25 * (S10 + S00 + Shh + Shn) *
               (log(c[ps10] / c00) - mat::bb * (g10 - g00));
    real F01 = 0.25 * (S01 + S00 + Shh + Snh) *
               (log(c[ps01] / c00) - mat::bb * (g01 - g00));
    real Fm0 = 0.25 * (S00 + Sm0 + Snh + Snn) *
               (log(c[psm0] / c00) - mat::bb * (gm0 - g00));
    real F0m = 0.25 * (S00 + S0m + Shn + Snn) *
               (log(c[ps0m] / c00) - mat::bb * (g0m - g00));

    real src10 = (F10 + F01 + Fm0 + F0m) /
                 (sim::dx * sim::dx); // the divergence evaluated by <10>
    ...
    real dc = para->dt * mat::Dl / (mat::Gamma * mat::muk0) * src10;
...
__global__ void compute_c_CALPHAD(real *c, real *phi, real *psi, real *T_fine,
  ...
  if (i > 0 && i < sim::Nx - 1 && j > 0 && j < sim::Ny - 1) {
    ...
    real dc = para->dt * mat::Dl / (mat::Gamma * mat::muk0) *
              (F10 + F01 + Fm0 + F0m) / (sim::dx * sim::dx);
...
__global__ void pull_back(real *c, real *phi, real *psi, real *c_next,
  ...
  i_next = max(i_next, 1);
  i_next = min(i_next, sim::Nx - 2); // FIXME by fixing types

  if (i_next < sim::Nx - 2)
    c_next[pos(i, j)] = c[pos(i_next, j)];
  else {
    c_next[pos(i, j)] = (param::simModel == param::Model::CALPHAD)
                            ? mat::cinf
                            : 1.0; // concentration
  }

  for (int a = 0; a < sim::num_orientation; a++)
                              ...
                              long int step) // flag
{
  ...
  int iT1 = i_left_end / sim::ratio_dxT_dx;
  // the first iT index for the left
  // border of the PF simulation domain
  int iTn = (i_left_end + sim::Nx - 3) / sim::ratio_dxT_dx;
  // the last iT index for the right border of the
  // PF simulation domain

  real integral = 0;

  if (iT >= iT1 && iT <= iTn && jT >= 1 && jT <= NyT - 2) {
    int i0 = sim::ratio_dxT_dx * iT + 1 - i_left_end;
    // in the T domain (iT1,jT), the
    // first i index of the PF domain
    int in = i0 + sim::ratio_dxT_dx - 1;
    // in the T domain (iT1,jT), the last i index of the PF domain
    if (i0 < 1)
      i0 = 1;
    if (in > sim::Nx - 2)
      in = sim::Nx - 2;
    ...
    int num_lattice = 0;
    for (int j = 1 + sim::ratio_dxT_dx * (jT - 1); j <= sim::ratio_dxT_dx * jT;
         j++) {
      for (int i = i0; i <= in; i++) {
...
__global__ void compute_temperature(real *T, real *T_next, real *dphidt,
  ...
  if (iT >= 1 && iT <= NxT - 2 && jT >= 1 && jT <= NyT - 2) {
    real laplacian = (T10 + Tm0 + T01 + T0m - 4.0 * T00) / pow2(sim::dxT);
    real dTdt = Vp_ramp(step * mat::tau0 * para->dt) * mat::tau0 /
                    (mat::S * mat::W0) * (T10 - Tm0) / (2.0 * sim::dxT) +
                laplacian * mat::DT / (mat::Gamma * mat::muk0);
    T_next[ps00] = T[ps00] + para->dtT * dTdt +
                   mat::Lcp * dphidt[ps00] /
                       (para->ratio_dt_dtT * pow2(sim::ratio_dxT_dx));
  }

  /////////////////// boundary condition
  if (iT == sim::iTmin)
    T_next[psT(0, jT)] = para->Tcold;
  else if (iT == sim::iTmax)
    T_next[psT(NxT - 1, jT)] = para->Thot;
  if (jT == sim::jTmin)
    T_next[psT(iT, 0)] = T_next[psT(iT, sim::jTmin)];
  else if (jT == sim::jTmax)
    T_next[psT(iT, NyT - 1)] = T_next[psT(iT, sim::jTmax)];
}
...
__host__ __device__ real get_local_psi(real *phi, int i, int j) {
  real psi = sim::num_orientation - 1;
  for (int a = 0; a < sim::num_orientation; a++)
    psi += phi[psN(i, j, a)];
  return psi;
}
...
void save_orientation(real *orientation) {
  ...
  if (fp == NULL) {
    ...
    for (int a = 0; a < sim::num_orientation; a++) {
      for (int i = 0; i < sim::num_axes; i++) {
        fprintf(fp, "%g\t", orientation[a * sim::num_axes + i]);
      }
      fprintf(fp, "\n");
    }
    fclose(fp);
...
void save_check_point(real *data, const char *field) // flag
{
  ...
  if (strcmp(field, "phi") == 0)
    num_slices = sim::num_orientation;
...
void load_orientation(real *orientation) {
  ...
  if (fp == NULL) {
    ...
    for (int a = 0; a < sim::num_orientation; a++) {
      for (int i = 0; i < sim::num_axes; i++) {
        fscanf(fp, "%lf\t", &orientation[a * sim::num_axes + i]);
      }
      fscanf(fp, "\n");
    }
    fclose(fp);
...
void AutoBlockSize(int *Bloc, int devNx, int devNy) {
  ...
  // sizes), reset to a standard efficient block size. 32 is standard for X
  // to match warp size (coalescing).
              ...
              real V_tip, real *h_c, real *h_phi) // flag
{
  ...
  fprintf(fp, "%-10d ", tip.i); // (8), position (integar) of the liquid
                                // front in the x direction
  fprintf(fp, "%-10d ", tip.j); // (9), position (integar) of the liquid
                                // front in the x direction
