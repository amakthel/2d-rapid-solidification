...
#define modelA 0
// interpolation between ε_s and ε_l (=f_l/c, change of free energy due to
// composition)
...
#define ifTFC 1
// temperature field calculation (TFC) or frozen temperature approximation
// (FTA)
#define if_noise 0

inline constexpr bool if_Vp_ramp{true};
// Vp_ramp()  x_pull_back() changed from directive to constexpr
...
#define i_tip_target (Nx - 268)
// position of the solid-liquid interface;  its distance to Nx
// should be > ~ 5*Dl/V
...
#define surface_tension (Gamma * 1e-9 * latentHeat2 / Tmelt)
...
#define psN(x, y, a) ((a) * Nx_all * Ny_all + (y) * Nx_all + (x))
// position in the phi matrix
...
#define gg(p) (1.875 * (p) - 1.25 * pow3(p) + 0.375 * pow5(p))
// 15.0/8.0 = 1.875
...
struct constants {
    ...
    real
        x_tip_history; // unit: W; tip position; used to calculate tip velocity;
...
int main(int argc, char **argv) {
    ...
    real *h_T = NULL,
         *h_T_fine =
             NULL; // not used in FTA, but appear in function definitions
    ...
    for (step = h_para->initial_step; step <= h_para->total_step; step++) {
        ...
        if (step == h_para->initial_step) {
            std::cout << "total steps for this simulation is: "
                      << h_para->total_step << std::endl;
        }
        set_fields_output(h_c, h_phi, h_psi, h_T, h_T_fine, h_para,
                          h_orientation, d_c, d_phi, d_psi, d_T, d_T_fine,
                          d_para, size_grid, size_grid_T, start_time, step,
        ...
        if (step % h_para->step_pull_back == 0) {
            set_pull_back(h_c, h_phi, h_psi, h_T, h_para, d_c, d_phi, d_psi,
                          d_T, d_para, size_grid, step);
            pull_back<<<numBlocks, threadsPerBlock>>>(
                d_c, d_phi, d_psi, d_c_next, d_phi_next, d_psi_next,
...
void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine,
    ...
    // random_seed
    // ); save_orientation(h_orientation); cudaMemcpy(d_c, h_c, size_grid,
    // cudaMemcpyHostToDevice); cudaMemcpy(d_phi, h_phi,
    ...
    cudaMemcpy(d_phi, h_phi, num_orientation * size_grid,
               cudaMemcpyHostToDevice);
                ...
                constants *para, int seed) // flag
{
    ...
    for (i = 0; i < num_nuclei_target; i++) {
        ...
        if (nuclei[3 * i + 2] == 0 && nuclei[3 * i + 1] >= j_border) {
            nuclei[3 * i + 1] = int((nuclei[3 * i + 1] - j_border) * j_border /
                                    (Ny - j_border));
        }
...
void set_nuclei_melt_pool(real *c, real *phi, real *psi, real *orientation,
    ...
    for (j = 0; j < Ny; j++) {
        for (i = 0; i < Nx; i++) {
            ...
            if (pow2(i - Nx) + pow2(j - Ny) >
                pow2(r_pool - initial_max_radius)) {
...
__global__ void compute_phi(real *c, real *phi, real *psi, real *T_fine,
    ...
    if (i > 0 && i < Nx - 1 && j > 0 && j < Ny - 1) {
        for (a = 0; a < num_orientation; a++) {
            ...
            real as, ak;
            real anisotropy =
                get_anisotropy(phi, orientation, &as, &ak, i, j, a);
...
/// add noise
        ...
        for (a = 0; a < num_orientation; a++) {
            ...
            real fluctuation =
                noise_amplitude * sqrt(para->dt) * (random - 0.5);
            phi_next[psN(i, j, a)] +=
                tanh((reference + fluctuation) / sqrt(2.0)) -
...
__global__ void compute_c_CALPHAD(real *c, real *phi, real *psi, real *T_fine,
    ...
    if (i > 0 && i < Nx - 1 && j > 0 && j < Ny - 1) {
        ...
        real f00 = 0.5 * (1.0 + g00) * Gsc(T00, c00) +
                   0.5 * (1.0 - g00) * Glc(T00, c00);
        real f10 = 0.5 * (1.0 + g10) * Gsc(T10, c10) +
                   0.5 * (1.0 - g10) * Glc(T10, c10);
        real f01 = 0.5 * (1.0 + g01) * Gsc(T01, c01) +
                   0.5 * (1.0 - g01) * Glc(T01, c01);
        real fm0 = 0.5 * (1.0 + gm0) * Gsc(Tm0, cm0) +
                   0.5 * (1.0 - gm0) * Glc(Tm0, cm0);
        real f0m = 0.5 * (1.0 + g0m) * Gsc(T0m, c0m) +
                   0.5 * (1.0 - g0m) * Glc(T0m, c0m);
        ...
        real dc = para->dt * Dl / (Gamma * muk0) * (F10 + F01 + Fm0 + F0m) /
                  (dx * dx);
                              ...
                              long int step) // flag
{
    ...
    if (iT >= iT1 && iT <= iTn && jT >= 1 && jT <= NyT - 2) {
        int i0 = ratio_dxT_dx * iT + 1 -
                 i_left_end; // in the T domain (iT1,jT), the
...
__host__ __device__ real G_Ti_Liq(real T) {
    if (T >= 1300.0 && T < 1941.0)
        return 369519.198 - 2554.0225 * T + 342.059267 * T * log(T) -
               163.409355e-3 * pow2(T) + 12.457117e-6 * pow3(T) -
               67034516.0 / T;
...
__host__ __device__ real Vp_ramp(real t) {
    if constexpr (if_Vp_ramp) {
        ...
        return Vp + 0.0107587 * t; // + 0.0005 * t;
    } else {
        return Vp;
    }
}
...
void set_parameters(constants *para) {
    ...
    real length = 50.0; // um
    para->length_sample = ceil(length * 1000 / (dx * SS * W0 * ratio_dxT_dx)) *
                          (dx * SS * W0 * ratio_dxT_dx); // nm
              ...
              real V_tip, real *h_c, real *h_phi) // flag
{
    ...
    if (std::filesystem::exists(tipPath)) {
        ...
        // (8)        (9)        (10)       (11)\n"); fprintf(fp, "step t[us]
        // Δtip       Ttip[K]    Vtip[m/s]  Vp[m/s]    tip.R[nm]  tip.i tip.j
        // tip.x      tip.y\n");
    ...
    //  fprintf(fp, "%-10g ", V_tip);  // (5), m/s, instantaneous velocity of
    //  the solid-liquid interface fprintf(fp, "%-10g ",
    ...
    // front in the x direction fprintf(fp, "%-10d ", tip.j);  // (9), position
    // (integar) of the liquid front in the x direction fprintf(fp, "%-10g ",
    // tip.x);  // (10), position (integar) of the liquid front in the x
    // direction fprintf(fp, "%-10g ", tip.y);  // (11), position (integar) of
    // the liquid front in the x direction
