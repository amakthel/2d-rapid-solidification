...
#define    BLOCK_SIZE_X 64 // 32x8 32x4 best; 64x4 good; 16x16 bad
#define    BLOCK_SIZE_Y 4

// #define    total_time    (100*1000) // ns
#define    num_fields_output 100 // output fields
#define    num_tip_output  10000 // output tip information
...
#define    latentHeat (22003) // J/mol, latent heat
...
__global__ void compute_c_CALPHAD( real *c, real *phi, real *psi, real *T_fine, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step, int i_offset_history, real l_offset_Vp );
__global__ void pull_back( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next, int i_offset, long int step );
...
void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                 real *d_c, real *d_phi, real *d_psi, real *d_T, real *d_T_fine, constants *d_para, real *d_orientation, curandState *d_state,
                 size_t size_grid, size_t size_grid_T, dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksT, dim3 threadsPerBlockT);
void set_fields_output( real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                        real *d_c, real *d_phi, real *d_psi, real *d_T, real *d_T_fine, constants *d_para, size_t size_grid, size_t size_grid_T,
...
void set_tip_output(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                    real *d_c, real *d_phi, real *d_psi, real *d_T, real *d_T_fine, constants *d_para, size_t size_grid, size_t size_grid_T,
                    real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock);
void set_check_point(real *h_c, real *h_phi, real *h_psi, real *h_T, real *d_c, real *d_phi, real *d_psi, real *d_T, constants *h_para,
...
void set_pull_back( real *h_c, real *h_phi, real *h_psi, real *h_T, constants *h_para,
                    real *d_c, real *d_phi, real *d_psi, real *d_T, constants *d_para, size_t size_grid, long int step);
...
{
    if (set_cuda_device() != 0)  // set the index of device and display the device information
    ...
    real *d_c, *d_phi, *d_psi, *d_orientation, *d_c_next, *d_phi_next, *d_psi_next;
    real *d_temp;
    curandState *d_state;
    ...
    cudaMalloc((void**)&d_orientation, num_axes*num_orientation*sizeof(real));
    ...
    set_initial(h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation,
                d_c, d_phi, d_psi, d_T, d_T_fine, d_para, d_orientation, d_state,
                size_grid, size_grid_T, numBlocks, threadsPerBlock, numBlocksT, threadsPerBlockT);

    // real delta = get_delta(h_para, i_tip_target, h_para->step, h_para->i_offset_history, h_para->l_offset_Vp);
    // printf("delta=%g, i_tip_target=%d, step=%d, i_offset_history=%d, l_offset_Vp=%g\n", 
    //     delta, i_tip_target, h_para->step, h_para->i_offset_history, h_para->l_offset_Vp);
    ...
    for (step=h_para->initial_step; step<=h_para->total_step; step++)
    {
        ...
        set_fields_output(h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation, 
                          d_c, d_phi, d_psi, d_T, d_T_fine, d_para, size_grid, size_grid_T, 
                          start_time, step, numBlocks, threadsPerBlock);
        set_tip_output( h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation, 
                        d_c, d_phi, d_psi, d_T, d_T_fine, d_para, size_grid, size_grid_T, 
                        start_time, step, numBlocks, threadsPerBlock); // flag
        set_check_point(h_c, h_phi, h_psi, h_T, d_c, d_phi, d_psi, d_T, h_para, size_grid, size_grid_T, step, 0);
        
        if ( step%h_para->step_pull_back==0 )
        {
            set_pull_back(h_c, h_phi, h_psi, h_T, h_para, d_c, d_phi, d_psi, d_T, d_para, size_grid, step );
            pull_back<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_c_next, d_phi_next, d_psi_next, h_para->i_offset, step );
            d_temp = d_c;          d_c = d_c_next;          d_c_next = d_temp;
            d_temp = d_phi;      d_phi = d_phi_next;      d_phi_next = d_temp;
            d_temp = d_psi;      d_psi = d_psi_next;      d_psi_next = d_temp;
        }

        //////////////////////////////////////////////////////////////////////////////////////////////////
        compute_phi<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_T_fine, d_c_next, d_phi_next, d_psi_next, 
                                                    d_orientation, d_state, d_para, step, h_para->i_offset_history, h_para->l_offset_Vp );
        compute_psi<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_c_next, d_phi_next, d_psi_next, 
                                                    d_orientation, d_state, d_para, step );
        #if (modelCALPHAD)
            compute_c_CALPHAD<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_T_fine, d_c_next, d_phi_next, d_psi_next, 
                                                    d_orientation, d_state, d_para, step, h_para->i_offset_history, h_para->l_offset_Vp );
        #else
            compute_c<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_c_next, d_phi_next, d_psi_next, 
                                                    d_orientation, d_state, d_para, step );
        #endif

        #if (ifTFC)
            integrate_phi<<<numBlocksT, threadsPerBlockT>>>( d_dphidt, d_psi, d_psi_next, d_para, h_para->i_left_end, step );
            for (sub_step=0; sub_step<h_para->ratio_dt_dtT; sub_step++)
            {
                compute_temperature<<<numBlocksT, threadsPerBlockT>>>( d_T, d_T_next, d_dphidt, d_para, step, sub_step );
                d_temp = d_T;          d_T = d_T_next;          d_T_next = d_temp;
            }
            interpolate_temperature<<<numBlocks, threadsPerBlock>>>(d_T, d_T_fine, d_para, h_para->i_left_end, step);
        #endif
    ...
    set_check_point(h_c, h_phi, h_psi, h_T, d_c, d_phi, d_psi, d_T, h_para, size_grid, size_grid_T, step, 1);
                 ...
                 size_t size_grid, size_t size_grid_T, dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksT, dim3 threadsPerBlockT)
{
    ...
    setup_kernel<<<numBlocks, threadsPerBlock>>>(random_seed,d_state);
    ...
    #if (if_load==0)
        cudaMemcpy(d_para, h_para, sizeof(constants), cudaMemcpyHostToDevice); // copy Tcold to the device
        initial<<<numBlocks, threadsPerBlock>>>( d_c, d_phi, d_psi, d_orientation, d_para );
        cudaMemcpy(h_orientation, d_orientation, num_axes*num_orientation*sizeof(real), cudaMemcpyDeviceToHost);
        save_orientation(h_orientation);

        #if (ifTFC)
            initial_temperature<<<numBlocksT, threadsPerBlockT>>>( d_T, d_para );
            interpolate_temperature<<<numBlocks, threadsPerBlock>>>(d_T, d_T_fine, d_para, h_para->i_left_end, h_para->initial_step);
        #endif
        
        // // if initial grains are randomly distributed
        // set_nuclei_melt_pool( h_c, h_phi, h_psi, h_orientation, h_para, random_seed );
        // save_orientation(h_orientation);
        ...
        // cudaMemcpy(d_orientation, h_orientation, num_axes*num_orientation*sizeof(real), cudaMemcpyHostToDevice);
    #else
                        ...
                        real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock)
{
    if ( step%h_para->step_fields==0 || step==h_para->initial_step ) // flag
    {
        ...
        cudaMemcpy(h_phi, d_phi, num_orientation*size_grid, cudaMemcpyDeviceToHost);
                    ...
                    real *d_c, real *d_phi, real *d_psi, real *d_T, constants *d_para, size_t size_grid, long int step)
{
    ...
    cudaMemcpy(d_para, h_para, sizeof(constants), cudaMemcpyHostToDevice);
        ...
        snprintf(offset_name, sizeof(offset_name), "data/offset_%3.2f.txt", Vp);
        ...
        cudaMemcpy(h_c, d_c, size_grid, cudaMemcpyDeviceToHost);
        char file_name[256];
        snprintf(file_name, sizeof(file_name), "data/Vp_%3.2f_c_history_temp.txt", Vp);
...
__device__ __forceinline__ void compute_boundary_individual(real *data, int i, int j, int a)
{
    ...
    if (j==jmin)      data[psN(i, 0,a)]     = data[psN(i, jmin,a)];
    else if (j==jmax) data[psN(i, Ny-1,a)] = data[psN(i, jmax,a)];
}
...
{
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "data/Vp_%3.2f_%s_%d.txt", Vp, field, index);
...
void save_T_full( real *T, constants *para, int index )
{
    int NxT_all = para->NxT_all;
    char file_name[256];
    sprintf(file_name, "data/T_full_%3.2f_%d.txt", Vp, index);
...
{
    FILE *fp = fopen("data/grain_final.txt","w");
    if (fp==NULL)
    {
...
{
    char filename[256];
    snprintf(filename, sizeof(filename), "data/orientation_%3.2f.txt", Vp);
...
{
    char fname[256];
    snprintf(fname, sizeof(fname), "data/check_point_%3.2f_%s.bin", Vp, field);
    int num_slices = 1;
...
void save_check_point_temperature( real *T, constants *para )
{
    int NxT_all = para->NxT_all;
    char fname[256];
    snprintf(fname, sizeof(fname), "data/check_point_%3.2f_temperature.bin", Vp);
    ofstream fp(fname);
    if ( !fp )
    {
        cout<< fname << " cannot be saved!!"<<endl;
...
{
    char fname[256];
    snprintf(fname, sizeof(fname), "%s/data/check_point_%3.2f_%s.bin", path_input, Vp, field);
    int num_slices = 1;
    if ( strcmp(field, "phi")==0 )  num_slices = num_orientation;
...
{
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "%s/data/orientation_%3.2f.txt", path_input, Vp);
...
void load_check_point_temperature( real *T, constants *para )
{
    ...
    char fname[256];
    snprintf(fname, sizeof(fname), "%s/data/check_point_%3.2f_temperature.bin", path_input, Vp);
    ifstream fp(fname);
...
__host__ __device__ real Vp_ramp(real t)
{
    if constexpr (if_Vp_ramp) {
        ...
        constexpr real switchoverTime{60.0};
        constexpr real lowSpeed{0.174};
        if (t < switchoverTime) {
            return Vp;
        } else {
            return lowSpeed;
...
void set_parameters(constants *para)
{
    para->initial_step = 0;
    para->step = 0;
    constexpr real DD = Dl*tau0/pow2(SS*W0); // dimensionless diffusivity
    ...
    #if (ifTFC)
        constexpr real DDT = DT*tau0/pow2(SS*W0*ratio_dxT_dx); // dimensionless thermal diffusivity
        constexpr real dtT = 0.9*dx*dx/(4.0*DDT);
...
{
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "parameters_%3.2f.txt", Vp);
    FILE * fp = fopen(file_name, "w");
    ...
    fprintf(fp, "%-13g Vp [m/s], initial pulling velocity \n", Vp);
    fprintf(fp, "%-13g Vp [m/s], current pulling velocity \n", Vp_ramp(step*tau0*h_para->dt));
    fprintf(fp, "\n");
...
{
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "%s/parameters_%3.2f.txt", path_input, Vp);
    FILE *fp = fopen(file_name, "r");
    if (fp == NULL)
    {
        printf("%s not found!\n", file_name);
...
void AutoBlockSize(int *Bloc, int devNx, int devNy)
{
    constexpr int best_metric = 100000; // Initialize with a large value
...
void save_tip( long int step, constants *h_para, real delta, real T_tip, TIP tip, real V_tip, real *h_c, real *h_phi) // flag
{
    ...
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "tip_%g.txt", Vp);
