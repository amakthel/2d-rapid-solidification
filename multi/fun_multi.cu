...
#define    modelA        0 // interpolation between ε_s and ε_l (=f_l/c, change of free energy due to composition)
#define    modelB        0 // interpolation between f_s and f_l (free energy)
#define    modelCALPHAD  1 // for concentrated alloys; a kind of model B
...
#define    if_1_eps     1 // 1: eps1, 0: (eps1, eps2)

// #define    Nx_all 2048
// #define    Ny_all 512
#define    Nx  Nx_all
#define    Ny  Ny_all // Ny-2 should be divided by ratio_dxT_dx=10
#define    i_tip_target (Nx-268) // position of the solid-liquid interface;  its distance to Nx should be > ~ 5*Dl/V
#define    di 1 // output result for every di points
#define    dj 1 // output result for every di points
#define    BLOCK_SIZE_X 32 // 32x8 32x4 best; 64x4 good; 16x16 bad
#define    BLOCK_SIZE_Y 8

// #define    total_time    (100*1000) // ns
#define    num_fields_output 100 // output fields
#define    num_tip_output  4 // output tip information
#define    num_check_point_output 100 // output check points
...
// #define    Gamma   (290.0)   // K*nm, Gibbs-Thomson coefficient
// #define    muk0    (1.0)  // nm/ns/K, interface kinetic coefficient, μm/s/K = 1e-6 nm/ns/K
// #define    tau0    ( pow2(SS*W0)/(Gamma*muk0) ) // ns, 5^2/196/0.5 = 0.2551 ns
// #define    Tmelt   (2750.0) // Tl = Tmelt + me*c_inf = 925.67 K
// #define    deltaT0 (Tmelt-2400.0) // K, equal to |me|*c_infty in the dilute limit
// #define    Dl      (4.0)    // nm^2/ns, liquid diffusion coefficient, μm^2/s = 1e-3 nm^2/ns
...
// #define    ke    0.13     // partition coefficient
// #define    bb     (0.5*log(ke))
// #define    d0     (Gamma/deltaT0)
// #define    lambda   (-bb*a1*SS*W0/d0/(1.0-ke))

// #define    latentHeat (25000.0) // J/mol, latent heat
// #define    cp (41.0) // J/mol/K, specific heat
// #define    Lcp (latentHeat/cp) // K, // (340.5)
// #define    molar_weigth 79.4 // g/mol, pure Al
// #define    density  6.8 // g/cm^3
// #define    num_moles (density*1e6/molar_weigth) // mol/m^3
...
#define    cp (40) // J/mol/K, specific heat, approximating between the solid and liquid heat capacities of the alloy
#define    Lcp (latentHeat/cp) // K, // (340.5)
...
#define    psN(x,y,a)     ((a)*Nx_all*Ny_all+(y)*Nx_all+(x)) // position in the phi matrix
...
#define    gg(p)    ( 1.875*(p)-1.25*pow3(p)+0.375*pow5(p) )  // 15.0/8.0 = 1.875
...
struct constants
{
    ...
    int i_offset_history_initial; // accumulated i_offset at the begining of the simulation
    int i_offset_history; // accumulated i_offset 
    int i_ignore; // for pull back: If i_ignore>0, ignore writing the history data; else write the data
    ...
    real l_offset_Vp_initial; // unit: W; lattice offset only due to pulling velocity
    real l_offset_Vp; // unit: W; lattice offset only due to pulling velocity

    ////////////////////////////// for TFC
    int i_left_end_initial; // index of the left border of the PF in the whole temperature field
    int i_left_end;
    ...
    ////////////////////////////// multi-GPU: rows [j_begin, j_end] (global indices)
    // owned/updated by the GPU that holds this copy of "para". The rest of the
    // grid on that GPU is either a halo (j_begin-1, j_end+1) or unused.
    int j_begin;
    int j_end;
};

__global__ void setup_kernel(unsigned long long seed,curandState *state);
__global__ void initial( real *c, real *phi, real *psi, real *orientation, constants *para );
__global__ void compute_phi( real *c, real *phi, real *psi, real *T_fine, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step, int i_offset_history, real l_offset_Vp );
__global__ void compute_psi( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step );
__global__ void compute_c( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step );
__global__ void compute_c_CALPHAD( real *c, real *phi, real *psi, real *T_fine, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step, int i_offset_history, real l_offset_Vp );
__global__ void pull_back( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next, int i_offset, long int step, int j_begin, int j_end );
__global__ void initial_temperature( real *T, constants *para );
__global__ void integrate_phi(real *dphidt, real *psi, real *psi_next, constants *para, int i_left_end, long int step);
__global__ void compute_temperature(real *T, real *T_next, real *dphidt, constants *para, long int step, int sub_step);
__global__ void interpolate_temperature(real *T, real *T_fine, constants *para, int i_left_end, long int step);
...
__host__ __device__ real get_delta(constants *para, int i, int i_offset_history, real l_offset_Vp);
__host__ __device__ void set_orientation( real *orientation, real alpha, real beta, real gamma );
...
// ============================ MULTI-GPU SUPPORT ============================ //
//
// Domain decomposition: the Nx_all x Ny_all grid is split into horizontal
// bands along y (the periodic direction). Each GPU owns global interior rows
// [jb[g], je[g]] and keeps a FULL-width copy of every field (Nx_all columns),
// so that the x-only operations (pull_back, tip detection, history output, the
// no-flux x boundary, the coarse temperature grid) work exactly as in the
// single-GPU code with no re-indexing. Neighbouring GPUs trade one ghost row
// each per step in a ring, which reproduces the periodic y BC (and degenerates
// to the original single-GPU periodic wrap when nGPU==1).
//////////////////////////////////////////////////////////////////////////////
// #define MAX_GPU 8
...
{
    int nGPU;
    int dev[MAX_GPU];   // CUDA device ordinals actually used
    int jb[MAX_GPU];    // first interior row (global index) owned by GPU g
    int je[MAX_GPU];    // last  interior row (global index) owned by GPU g

    // Per-GPU device arrays. Each is full size (Nx_all*Ny_all, or *num_orientation
    // for phi), only the band [jb-1 .. je+1] is kept valid on a given GPU.
    real *d_c[MAX_GPU],      *d_phi[MAX_GPU],      *d_psi[MAX_GPU];
    real *d_c_next[MAX_GPU], *d_phi_next[MAX_GPU], *d_psi_next[MAX_GPU];
    real *d_orientation[MAX_GPU];
    curandState *d_state[MAX_GPU];
    constants *d_para[MAX_GPU];
    constants h_para_g[MAX_GPU];   // host mirror carrying each GPU's own band

    // TFC (temperature field) arrays -- only allocated when ifTFC is enabled.
    real *d_T[MAX_GPU], *d_T_next[MAX_GPU], *d_dphidt[MAX_GPU], *d_T_fine[MAX_GPU];
};

int  set_cuda_devices(MultiGPU *mg);                 // discover + init up to MAX_GPU GPUs
void mg_swap(real *a[MAX_GPU], real *b[MAX_GPU], int n);
void halo_exchange(MultiGPU *mg);                    // ring exchange of c, phi, psi ghost rows
void halo_exchange_one(MultiGPU *mg, real *dptr[MAX_GPU], int nslice);
void gather_field (MultiGPU *mg, real *h, real *dptr[MAX_GPU], int nslice);
void scatter_field(MultiGPU *mg, real *dptr[MAX_GPU], real *h, int nslice);

void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                 MultiGPU *mg, size_t size_grid, size_t size_grid_T,
                 dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksT, dim3 threadsPerBlockT);
void set_fields_output( real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                        MultiGPU *mg, size_t size_grid, size_t size_grid_T,
                        real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock);
void set_tip_output(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                    MultiGPU *mg, size_t size_grid, size_t size_grid_T,
                    real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock);
void set_check_point(real *h_c, real *h_phi, real *h_psi, real *h_T, MultiGPU *mg, constants *h_para,
                    size_t size_grid, size_t size_grid_T, long int step, int final_step);
void set_pull_back( real *h_c, real *h_phi, real *h_psi, real *h_T, constants *h_para,
                    MultiGPU *mg, size_t size_grid, long int step);

void set_nuclei( real *c, real *phi, real *psi, real *orientation, constants *para, int seed );
void set_nuclei_melt_pool( real *c, real *phi, real *psi, real *orientation, constants *para, int seed );
...
{
    ////////////////////////////////////////////////////////////////////////////
    // Discover and initialise up to MAX_GPU GPUs. mg holds every per-GPU handle.
    ////////////////////////////////////////////////////////////////////////////
    MultiGPU mg;
    if (set_cuda_devices(&mg) != 0)
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Split interior rows 1..Ny_all-2 into nGPU contiguous bands (extra rows go
    // to the lowest-index GPUs). Each band -> jb[g]..je[g] (global row indices).
    ////////////////////////////////////////////////////////////////////////////
    {
        int interior = Ny_all - 2;
        int base = interior / mg.nGPU;
        int rem  = interior % mg.nGPU;
        int row  = 1;
        for (int g=0; g<mg.nGPU; g++)
        {
            int cnt = base + (g < rem ? 1 : 0);
            mg.jb[g] = row;
            mg.je[g] = row + cnt - 1;
            row += cnt;
            printf("GPU %d (device %d): rows %d..%d (%d rows)\n",
                   g, mg.dev[g], mg.jb[g], mg.je[g], cnt);
        }
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////// matrix definitions
    size_t size_grid = Nx_all*Ny_all*sizeof(real);
    real *h_c, *h_phi, *h_psi, *h_orientation; // psi: num_orientation - 1 + Σ phi
    ...
    ////////////////////////////////////////////////////////////////////////////
    // Allocate a full-width copy of every field on every GPU, and store this
    // GPU's band inside its own device-side "para" (used by the kernel guards).
    ////////////////////////////////////////////////////////////////////////////
    for (int g=0; g<mg.nGPU; g++)
    {
        cudaSetDevice(mg.dev[g]);
        cudaMalloc((void**)&mg.d_c[g],         size_grid);
        cudaMalloc((void**)&mg.d_phi[g],       num_orientation*size_grid);
        cudaMalloc((void**)&mg.d_psi[g],       size_grid);
        cudaMalloc((void**)&mg.d_c_next[g],    size_grid);
        cudaMalloc((void**)&mg.d_phi_next[g],  num_orientation*size_grid);
        cudaMalloc((void**)&mg.d_psi_next[g],  size_grid);
        cudaMalloc((void**)&mg.d_orientation[g], num_axes*num_orientation*sizeof(real));
        cudaMalloc((void**)&mg.d_state[g],     Nx_all*Ny_all*sizeof(curandState));
        cudaMalloc((void**)&mg.d_para[g],      sizeof(constants));

        mg.h_para_g[g] = *h_para;            // copy all the shared parameters ...
        mg.h_para_g[g].j_begin = mg.jb[g];   // ... then stamp in this GPU's band
        mg.h_para_g[g].j_end   = mg.je[g];
        cudaMemcpy(mg.d_para[g], &mg.h_para_g[g], sizeof(constants), cudaMemcpyHostToDevice);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////// TFC
    size_t size_grid_T=0;
    real *h_T=NULL, *h_T_fine=NULL; // not used in FTA, but appear in function definitions
    ...
    dim3 numBlocksT(1, 1);
        ...
        // Temperature field is small and its PF<->T coupling spans the whole
        // domain in y, so (for simplicity and provable correctness) it is solved
        // REDUNDANTLY, in full, on every GPU. Each step we make psi_next globally
        // available (gather+scatter) so every GPU can integrate over the whole
        // grid; T and T_fine then come out identical on all GPUs. This favours
        // clarity over communication volume -- fine because TFC is off by default;
        // a banded T decomposition (aligned to ratio_dxT_dx) is the natural
        // optimisation if this path becomes performance critical.
        for (int g=0; g<mg.nGPU; g++)
        {
            cudaSetDevice(mg.dev[g]);
            cudaMalloc((void**)&mg.d_T[g],      size_grid_T);
            cudaMalloc((void**)&mg.d_T_next[g], size_grid_T);
            cudaMalloc((void**)&mg.d_dphidt[g], size_grid_T);
            cudaMalloc((void**)&mg.d_T_fine[g], size_grid);
        }

        threadsPerBlockT = dim3(BLOCK_SIZE_T_X, BLOCK_SIZE_T_Y);
        numBlocksT = dim3(h_para->NxT_all/threadsPerBlockT.x, h_para->NyT_all/threadsPerBlockT.y);
    #endif

    ////////////////////////////////////////////////////////////////////////////////////////////////////// initial condition
    set_initial(h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation,
                &mg, size_grid, size_grid_T, numBlocks, threadsPerBlock, numBlocksT, threadsPerBlockT);

    ////////////////////////////////////////////////////////////////////////////////////////////////////// iterations
    for (step=h_para->initial_step; step<=h_para->total_step; step++)
    {
        if (step == h_para->initial_step) {
            std::cout << "total steps for this simulation is: " << h_para->total_step << std::endl;
        }
        set_fields_output(h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation,
                          &mg, size_grid, size_grid_T, start_time, step, numBlocks, threadsPerBlock);
        set_tip_output( h_c, h_phi, h_psi, h_T, h_T_fine, h_para, h_orientation,
                        &mg, size_grid, size_grid_T, start_time, step, numBlocks, threadsPerBlock); // flag
        set_check_point(h_c, h_phi, h_psi, h_T, &mg, h_para, size_grid, size_grid_T, step, 0);

        if ( step%h_para->step_pull_back==0 )
        {
            // i_offset is GLOBAL (from the global tip) so every GPU shifts by the
            // same amount; each GPU pulls back only its own band.
            set_pull_back(h_c, h_phi, h_psi, h_T, h_para, &mg, size_grid, step );
            for (int g=0; g<mg.nGPU; g++)
            {
                cudaSetDevice(mg.dev[g]);
                pull_back<<<numBlocks, threadsPerBlock>>>( mg.d_c[g], mg.d_phi[g], mg.d_psi[g],
                        mg.d_c_next[g], mg.d_phi_next[g], mg.d_psi_next[g],
                        h_para->i_offset, step, mg.jb[g], mg.je[g] );
            }
            mg_swap(mg.d_c,   mg.d_c_next,   mg.nGPU);
            mg_swap(mg.d_phi, mg.d_phi_next, mg.nGPU);
            mg_swap(mg.d_psi, mg.d_psi_next, mg.nGPU);
        }

        //////////////////////////////////////////////////////////////////////////////////////////////////
        // Refresh ghost rows BEFORE the compute step (this also covers the case
        // where pull_back just swapped in fresh, un-haloed arrays).
        halo_exchange(&mg);

        for (int g=0; g<mg.nGPU; g++)
        {
            cudaSetDevice(mg.dev[g]);
            compute_phi<<<numBlocks, threadsPerBlock>>>( mg.d_c[g], mg.d_phi[g], mg.d_psi[g], mg.d_T_fine[g],
                    mg.d_c_next[g], mg.d_phi_next[g], mg.d_psi_next[g],
                    mg.d_orientation[g], mg.d_state[g], mg.d_para[g], step, h_para->i_offset_history, h_para->l_offset_Vp );
            compute_psi<<<numBlocks, threadsPerBlock>>>( mg.d_c[g], mg.d_phi[g], mg.d_psi[g],
                    mg.d_c_next[g], mg.d_phi_next[g], mg.d_psi_next[g],
                    mg.d_orientation[g], mg.d_state[g], mg.d_para[g], step );
            #if (modelCALPHAD)
                compute_c_CALPHAD<<<numBlocks, threadsPerBlock>>>( mg.d_c[g], mg.d_phi[g], mg.d_psi[g], mg.d_T_fine[g],
                        mg.d_c_next[g], mg.d_phi_next[g], mg.d_psi_next[g],
                        mg.d_orientation[g], mg.d_state[g], mg.d_para[g], step, h_para->i_offset_history, h_para->l_offset_Vp );
            #else
                compute_c<<<numBlocks, threadsPerBlock>>>( mg.d_c[g], mg.d_phi[g], mg.d_psi[g],
                        mg.d_c_next[g], mg.d_phi_next[g], mg.d_psi_next[g],
                        mg.d_orientation[g], mg.d_state[g], mg.d_para[g], step );
            #endif
        }
        // wait for all devices to finish this step before swapping / next halo
        for (int g=0; g<mg.nGPU; g++) { cudaSetDevice(mg.dev[g]); cudaDeviceSynchronize(); }

        #if (ifTFC)
            // make psi_next globally available, then solve T redundantly on each GPU
            gather_field(&mg, h_psi, mg.d_psi_next, 1);
            scatter_field(&mg, mg.d_psi_next, h_psi, 1);
            for (int g=0; g<mg.nGPU; g++)
            {
                cudaSetDevice(mg.dev[g]);
                integrate_phi<<<numBlocksT, threadsPerBlockT>>>( mg.d_dphidt[g], mg.d_psi[g], mg.d_psi_next[g], mg.d_para[g], h_para->i_left_end, step );
            }
            for (sub_step=0; sub_step<h_para->ratio_dt_dtT; sub_step++)
            {
                ...
                {
                    cudaSetDevice(mg.dev[g]);
                    compute_temperature<<<numBlocksT, threadsPerBlockT>>>( mg.d_T[g], mg.d_T_next[g], mg.d_dphidt[g], mg.d_para[g], step, sub_step );
                }
                mg_swap(mg.d_T, mg.d_T_next, mg.nGPU);
            }
            for (int g=0; g<mg.nGPU; g++)
            {
                cudaSetDevice(mg.dev[g]);
                interpolate_temperature<<<numBlocks, threadsPerBlock>>>(mg.d_T[g], mg.d_T_fine[g], mg.d_para[g], h_para->i_left_end, step);
            }
            for (int g=0; g<mg.nGPU; g++) { cudaSetDevice(mg.dev[g]); cudaDeviceSynchronize(); }
        #endif
        ...
        h_para->l_offset_Vp += Vp_ramp(step*tau0*h_para->dt)*h_para->dt*tau0/(SS*W0);
        //////////////////////////////////////////////////////////////////////////////////////////////////
        if ( clock()/(1.0*CLOCKS_PER_SEC) - start_time > run_time*3600.0-1000.0 )
            break;
    }

    set_check_point(h_c, h_phi, h_psi, h_T, &mg, h_para, size_grid, size_grid_T, step, 1);
    ...
    for (int g=0; g<mg.nGPU; g++)
    {
        ...
        cudaFree(mg.d_phi[g]);
        cudaFree(mg.d_psi[g]);
        cudaFree(mg.d_c_next[g]);
        cudaFree(mg.d_phi_next[g]);
        cudaFree(mg.d_psi_next[g]);
        cudaFree(mg.d_orientation[g]);
        cudaFree(mg.d_para[g]);
        cudaFree(mg.d_state[g]);
        #if (ifTFC)
            cudaFree(mg.d_T[g]);
            cudaFree(mg.d_T_fine[g]);
            cudaFree(mg.d_T_next[g]);
            cudaFree(mg.d_dphidt[g]);
        #endif
    }
...
//////////////////////////////////////////////////////////////////////////////
// ===================== MULTI-GPU HELPER IMPLEMENTATIONS ==================== //
//////////////////////////////////////////////////////////////////////////////

// Swap two arrays of nGPU device pointers (used for the ping-pong buffers).
void mg_swap(real *a[MAX_GPU], real *b[MAX_GPU], int n)
{
    for (int g=0; g<n; g++) { real *t=a[g]; a[g]=b[g]; b[g]=t; }
}

// Copy one full row (Nx_all reals) from row srcRow on GPU srcG to row dstRow on
// GPU dstG. cudaMemcpyPeer works with or without peer access (it stages through
// the host automatically when P2P is unavailable), so this is always correct.
static inline void copy_row(MultiGPU *mg, int dstG, real *dstBase, int dstRow,
                            int srcG, real *srcBase, int srcRow)
{
    cudaMemcpyPeer(dstBase + (size_t)dstRow*Nx_all, mg->dev[dstG],
                   srcBase + (size_t)srcRow*Nx_all, mg->dev[srcG],
                   (size_t)Nx_all*sizeof(real));
}

// Ring halo exchange for one field with nslice slices (1 for c/psi,
// num_orientation for phi). GPU g's upper ghost row (je[g]+1) is filled from the
// first interior row of the GPU above it; its lower ghost row (jb[g]-1) from the
// last interior row of the GPU below it. The (g+1)%G / (g-1+G)%G wrap-around is
// exactly the periodic y BC for the top/bottom GPUs, and reduces to the original
// single-GPU periodic copy when nGPU==1.
void halo_exchange_one(MultiGPU *mg, real *dptr[MAX_GPU], int nslice)
{
    int G = mg->nGPU;
    for (int a=0; a<nslice; a++)
    {
        size_t soff = (size_t)a*Nx_all*Ny_all;
        for (int g=0; g<G; g++)
        {
            int up = (g+1)%G;     // owns the rows just above g
            int dn = (g-1+G)%G;   // owns the rows just below g
            copy_row(mg, g, dptr[g]+soff, mg->je[g]+1,  up, dptr[up]+soff, mg->jb[up]);
            copy_row(mg, g, dptr[g]+soff, mg->jb[g]-1,  dn, dptr[dn]+soff, mg->je[dn]);
        }
    }
}

void halo_exchange(MultiGPU *mg)
{
    halo_exchange_one(mg, mg->d_c,   1);
    halo_exchange_one(mg, mg->d_phi, num_orientation);
    halo_exchange_one(mg, mg->d_psi, 1);
    for (int g=0; g<mg->nGPU; g++) { cudaSetDevice(mg->dev[g]); cudaDeviceSynchronize(); }
}

// Assemble the full host array h from the interior bands of every GPU, plus the
// two global ghost rows (0 and Ny_all-1), which live on GPU 0 / the last GPU.
void gather_field(MultiGPU *mg, real *h, real *dptr[MAX_GPU], int nslice)
{
    for (int a=0; a<nslice; a++)
    {
        ...
        {
            cudaSetDevice(mg->dev[g]);
            int nrow = mg->je[g]-mg->jb[g]+1;
            cudaMemcpy(h + soff + (size_t)mg->jb[g]*Nx_all,
                       dptr[g] + soff + (size_t)mg->jb[g]*Nx_all,
                       (size_t)nrow*Nx_all*sizeof(real), cudaMemcpyDeviceToHost);
        }
        // Global y-ghost rows 0 and Ny_all-1: fill by the periodic wrap on the
        // host (row 0 <- row Ny_all-2, row Ny_all-1 <- row 1), exactly matching
        // the single-GPU boundary treatment. The interior rows they copy from
        // are already gathered above, so this is independent of halo timing.
        memcpy(h + soff,                                     // row 0
               h + soff + (size_t)(Ny_all-2)*Nx_all,
               (size_t)Nx_all*sizeof(real));
        memcpy(h + soff + (size_t)(Ny_all-1)*Nx_all,         // row Ny_all-1
               h + soff + (size_t)1*Nx_all,
               (size_t)Nx_all*sizeof(real));
    }
}

// Broadcast a full host array to every GPU (used at init and after a load).
void scatter_field(MultiGPU *mg, real *dptr[MAX_GPU], real *h, int nslice)
{
    size_t bytes = (size_t)nslice*Nx_all*Ny_all*sizeof(real);
    for (int g=0; g<mg->nGPU; g++)
    {
        cudaSetDevice(mg->dev[g]);
        cudaMemcpy(dptr[g], h, bytes, cudaMemcpyHostToDevice);
    }
}

// Re-push the shared parameters to every device (preserving each GPU's band).
// Needed after load_parameters() changes h_para.
static void push_para(MultiGPU *mg, constants *h_para)
{
    for (int g=0; g<mg->nGPU; g++)
    {
        mg->h_para_g[g] = *h_para;
        mg->h_para_g[g].j_begin = mg->jb[g];
        mg->h_para_g[g].j_end   = mg->je[g];
        cudaSetDevice(mg->dev[g]);
        cudaMemcpy(mg->d_para[g], &mg->h_para_g[g], sizeof(constants), cudaMemcpyHostToDevice);
    }
...
void set_initial(real *h_c, real *h_phi, real *h_psi, real *h_T, real *h_T_fine, constants *h_para, real *h_orientation,
                 ...
                 dim3 numBlocks, dim3 threadsPerBlock, dim3 numBlocksT, dim3 threadsPerBlockT)
{
    ensure_directory("data"); // create the folder if not exist

    // Seed the RNG on every GPU. Because setup_kernel seeds each cell by pos(i,j)
    // over the full grid, every GPU ends up with the SAME per-cell state as the
    // original single-GPU run -> results are reproducible regardless of nGPU.
    for (int g=0; g<mg->nGPU; g++)
    {
        cudaSetDevice(mg->dev[g]);
        setup_kernel<<<numBlocks, threadsPerBlock>>>(random_seed, mg->d_state[g]);
    }

    #if (if_load==0)
        push_para(mg, h_para);
        // The initial condition is a deterministic function of (i,j), so each GPU
        // fills its FULL grid identically -> every band and every ghost row is
        // valid immediately, no scatter needed.
        for (int g=0; g<mg->nGPU; g++)
        {
            cudaSetDevice(mg->dev[g]);
            initial<<<numBlocks, threadsPerBlock>>>( mg->d_c[g], mg->d_phi[g], mg->d_psi[g], mg->d_orientation[g], mg->d_para[g] );
            #if (ifTFC)
                initial_temperature<<<numBlocksT, threadsPerBlockT>>>( mg->d_T[g], mg->d_para[g] );
                interpolate_temperature<<<numBlocks, threadsPerBlock>>>(mg->d_T[g], mg->d_T_fine[g], mg->d_para[g], h_para->i_left_end, h_para->initial_step);
            #endif
        }
        cudaSetDevice(mg->dev[0]);
        cudaMemcpy(h_orientation, mg->d_orientation[0], num_axes*num_orientation*sizeof(real), cudaMemcpyDeviceToHost);
        ...
        // Broadcast the loaded fields (full grid) to every GPU, then refresh the
        // per-device parameters (band + loaded dynamic state).
        push_para(mg, h_para);
        scatter_field(mg, mg->d_c,   h_c,   1);
        scatter_field(mg, mg->d_phi, h_phi, num_orientation);
        scatter_field(mg, mg->d_psi, h_psi, 1);
        for (int g=0; g<mg->nGPU; g++)
        {
            cudaSetDevice(mg->dev[g]);
            cudaMemcpy(mg->d_orientation[g], h_orientation, num_axes*num_orientation*sizeof(real), cudaMemcpyHostToDevice);
            #if (ifTFC)
                cudaMemcpy(mg->d_T[g], h_T, size_grid_T, cudaMemcpyHostToDevice);
                interpolate_temperature<<<numBlocks, threadsPerBlock>>>(mg->d_T[g], mg->d_T_fine[g], mg->d_para[g], h_para->i_left_end, h_para->initial_step);
            #endif
        }
    #endif
                        ...
                        real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock)
{
    ...
    {
        gather_field(mg, h_c,   mg->d_c,   1);
        gather_field(mg, h_phi, mg->d_phi, num_orientation);
        gather_field(mg, h_psi, mg->d_psi, 1);
                    ...
                    size_t size_grid, size_t size_grid_T, long int step, int final_step)
{
    if ( step%h_para->step_check_point==0 || final_step==1 )
    {
        ...
        #if (ifTFC)
            cudaSetDevice(mg->dev[0]); // T is replicated; GPU 0's copy is authoritative
            cudaMemcpy(h_T, mg->d_T[0], size_grid_T, cudaMemcpyDeviceToHost);
...
void set_pull_back( real *h_c, real *h_phi, real *h_psi, real *h_T, constants *h_para,
                    MultiGPU *mg, size_t size_grid, long int step)
{
    // The dendrite tip is global, so assemble the full psi and find the global
    // tip; i_offset is then the same on every GPU.
    gather_field(mg, h_psi, mg->d_psi, 1);
    h_para->i_offset = get_itip_all_grain( h_psi ) - i_tip_target;
    h_para->i_offset_history += h_para->i_offset;

    #if (ifTFC)
    h_para->i_left_end  = h_para->i_left_end_initial + h_para->i_offset_history - int(h_para->l_offset_Vp/dx);
    #endif
        ...
        fprintf(file_ioffset, "%ld\t%d\t%d\t%d\n", step, h_para->i_offset, h_para->i_offset_history, h_para->step_pull_back );
        fclose(file_ioffset);

        ///////////////////////// record the history data
        gather_field(mg, h_c, mg->d_c, 1);
        char file_name[256];
        snprintf(file_name, sizeof(file_name), "%s/data/c_history_temp.txt", path_input);
        FILE *file_history = fopen(file_name, "a");
        h_para->i_ignore -= h_para->i_offset; // if i_ignore>0, ignore writing; else write the data
        int i,j;
        for (i=0; i<h_para->i_offset; i++)
        {
            if ( (i + h_para->i_offset_history)%di==0 )
            {
                ...
                h_para->i_ignore--;
                if (h_para->i_ignore<0)     h_para->i_ignore = 0; // for the case that i_offset<0 
            }
        }
        fclose(file_history);
    #endif
...
__host__ __device__ void set_orientation( real *orientation, real alpha, real beta, real gamma )
{
    ...
    orientation[1] = sin(alpha)*sin(beta)*cos(gamma) - cos(alpha)*sin(gamma);
    orientation[2] = cos(alpha)*sin(beta)*cos(gamma) + sin(alpha)*sin(gamma);

    orientation[3] = cos(beta)*sin(gamma);
    orientation[4] = sin(alpha)*sin(beta)*sin(gamma) + cos(alpha)*cos(gamma);
    orientation[5] = cos(alpha)*sin(beta)*sin(gamma) - sin(alpha)*cos(gamma);
...
void generate_random_orientation( real *orientation, int seed ) // for polycrystalline
{
    srand(seed);
...
void set_nuclei( real *c, real *phi, real *psi, real *orientation, constants *para, int seed )  // flag
{
    ...
    for (i=0; i<num_nuclei_target; i++)
    {
        ...
        {
            nuclei[3*i+1] = int( (nuclei[3*i+1]-j_border) * j_border/(Ny-j_border) ) ;
        }
        if ( nuclei[3*i+2]==1 && nuclei[3*i+1]<j_border )
        {
            nuclei[3*i+1] = int( nuclei[3*i+1]*(Ny-j_border)/j_border ) + j_border;
        }
    }

    // if (i,j) is closest to the center of nucleus l, then (i,j) is within nucleus l
    ///////////////////////////////////
    ...
    for (j=0; j<Ny; j++)
    {
        for (i=0; i<Nx; i++)
        {
            ...
            for (l=0; l<num_nuclei_target; l++)
            {
                ...
                dis_jlow = sqrt( pow2( i-nuclei[3*l+0] ) + pow2( j-(nuclei[3*l+1]-(Ny-2)) ) ); // nuclei on the upper mirror
                dis_jup = sqrt( pow2( i-nuclei[3*l+0] ) + pow2( j-(nuclei[3*l+1]+(Ny-2)) ) ); // nuclei on the lower mirror

                dis_temp = min_dis;

                if (min_dis > dis) min_dis = dis;
                if (min_dis > dis_jlow)  min_dis = dis_jlow;
                if (min_dis > dis_jup)  min_dis = dis_jup;
...
void set_nuclei_melt_pool( real *c, real *phi, real *psi, real *orientation, constants *para, int seed )
{
    ...
    int ind=0;
    int num_nuclei;
    real random, possibility, num_lattice_outside_ellipse = Nx*Ny - PI*pow2(r_pool)/4.0;
        ...
        for (i=0; i<Nx; i++)
        {
            if ( pow2(i-Nx) + pow2(j-Ny) > pow2( r_pool ) )
            {
                ...
                if ( random < possibility )
                {
                    nuclei[3*ind+0] = i;
                    nuclei[3*ind+1] = j;
                    nuclei[3*ind+2] = (int) floor( random/(possibility/num_orientation) );
                    ind++;
                }
            }
        }
    ...
    for (j=0; j<Ny; j++)
    {
        for (i=0; i<Nx; i++)
        {
            ...
            if ( pow2(i-Nx) + pow2(j-Ny) > pow2( r_pool - initial_max_radius ) )
            {
                ...
                {
                    dis = sqrt( pow2( i-nuclei[3*l+0] ) + pow2( j-nuclei[3*l+1] ) );
                    dis_jlow = sqrt( pow2( i-nuclei[3*l+0] ) + pow2( j-(nuclei[3*l+1]-(Ny-2)) ) ); // nucleus on the upper mirror
                    dis_jup = sqrt( pow2( i-nuclei[3*l+0] ) + pow2( j-(nuclei[3*l+1]+(Ny-2)) ) ); // nucleus on the lower mirror
...
__global__ void initial( real *c, real *phi, real *psi, real *orientation, constants *para )  // flag
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int ps = pos(i,j);
    ...
    // real pp = -tanh( (i-itip)*dx/sqrt(2.0) ); // consider going back for this part ...
    // phi[ps] = pp;
    // real cc = exp(bb*(1.0 + gg(pp))); // 0.3184*tanh( ((i-itip)*dx-0.08)/0.67 ) + 0.6816;
...
__device__ real get_anisotropy(real *phi, real *orientation, real *as, real *ak, int i, int j, int a)
{
    int psa = psN(i,j,a);
    real phi00 = phi[psa];
    ...
    real laplacian_phi = 2.0/3.0 * ( dphi_dx_dx + dphi_dy_dy );
         laplacian_phi+= 1.0/3.0 * (phi11+phim1+phi1m+phimm - 4.0*phi00)/(2.0*dx*dx) ;

    real dtheta_dx, dtheta_dy, cos_theta, sin_theta;
    if ( abs_gradient_phi>1.0e-15 )
    {
        dtheta_dx = (dphi_dx*dphi_dx_dy-dphi_dy*dphi_dx_dx)/pow2(abs_gradient_phi);
        dtheta_dy = (dphi_dx*dphi_dy_dy-dphi_dy*dphi_dx_dy)/pow2(abs_gradient_phi);
    ...
    real dnp4_dtheta_1 = 4.0*( pow3(nxp)*dnxp_dtheta_1 + pow3(nyp)*dnyp_dtheta_1 + pow3(nzp)*dnzp_dtheta_1);
    real dnp4_dtheta_2  = 3.0*pow2(nxp*dnxp_dtheta_1) + pow3(nxp)*dnxp_dtheta_2;
    ...
    #else
        *ak = 1.0 + epk1*(np4 - 0.6);
        *as = 1.0 + eps1*(np4 - 0.6) + eps2*( 3.0*np4 + 66.0*pow2(nxp*nyp*nzp) -17.0/7.0 );

        // np2 = pow2(nxp*nyp*nzp)
        real dnp2_dtheta_1 = 2.0*nxp*nyp*nzp*( dnxp_dtheta_1*nyp*nzp + nxp*dnyp_dtheta_1*nzp + nxp*nyp*dnzp_dtheta_1 );
        real dnp2_dtheta_2  = pow2(dnxp_dtheta_1*nyp*nzp) + pow2(nxp*dnyp_dtheta_1*nzp) + pow2(nxp*nyp*dnzp_dtheta_1);
             dnp2_dtheta_2 += nxp*dnxp_dtheta_2*pow2(nyp*nzp) + nyp*dnyp_dtheta_2*pow2(nxp*nzp) + nzp*dnzp_dtheta_2*pow2(nxp*nyp);
             dnp2_dtheta_2 += 4.0*nxp*nyp*nzp*( dnxp_dtheta_1*dnyp_dtheta_1*nzp + nxp*dnyp_dtheta_1*dnzp_dtheta_1 + dnxp_dtheta_1*nyp*dnzp_dtheta_1 );
        dnp2_dtheta_2 *= 2.0;

        real da_dtheta_1 = (eps1+3.0*eps2)*dnp4_dtheta_1 + 66.0*eps2*dnp2_dtheta_1;
        real da_dtheta_2 = (eps1+3.0*eps2)*dnp4_dtheta_2 + 66.0*eps2*dnp2_dtheta_2;
    #endif

    real anisotropy = pow2(*as)*laplacian_phi + 2.0*(*as)*da_dtheta_1*( dtheta_dx*dphi_dx + dtheta_dy*dphi_dy )
                    + ( da_dtheta_2*(*as) + pow2(da_dtheta_1) )*( dtheta_dy*dphi_dx - dtheta_dx*dphi_dy );

    return anisotropy;
}

// V100 has 256^2 registers per block; a real needs 2 registers; a int needs 1 register
__global__ void compute_phi( real *c, real *phi, real *psi, real *T_fine, real *c_next, real *phi_next, real *psi_next,    // flag
                            real *orientation, curandState *state, constants *para, long int step, int i_offset_history, real l_offset_Vp )
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    if (i>0 && i<Nx-1 && j>=para->j_begin && j<=para->j_end)
    {
        for (a=0; a<num_orientation; a++)
        {
            int psa = psN(i,j,a);
            ...
            real term = phi00 - pow3(phi00) + anisotropy;
                ...
                real omega = -6.0*lambda*( 2*ke + (ke-1.0)/bb*delta ); // coupling between neighboring grains
                real T00 = delta*deltaT0 + Tmelt;
            #endif

            #if (modelA)
                term += -lambda*dgg_dp(phi00)*( c00 + delta*exp(bb*(1.0+gg(phi00))) ); // 
                term += -omega*0.5*(1.0+phi00)*sum_phi; // coupling between neighboring grains
            #elif (modelB)
                term += -lambda*dgg_dp(phi00)*( c00 + delta*(ke-1.0)/(2.0*bb) ); // 
                term += -omega*0.5*(1.0+phi00)*sum_phi; // coupling between neighboring grains
            #elif (modelCALPHAD)
                term += ( Gl(T00,c00) - Gs(T00,c00) )*dgg_dp(phi00)/(2.0*hh*v0); // 
            #endif
        ...
        /////////////////////////////////////////////////////////////////////////////////// add noise
            ...
            for (a=0; a<num_orientation; a++)
            {
                ...
                phi_next[psN(i,j,a)] += tanh((reference+fluctuation)/sqrt(2.0))-tanh(reference/sqrt(2.0));
            }
            state[ps] = localState;
        #endif
    } // i,j
...
__device__ __forceinline__ void compute_boundary_individual(real *data, int i, int j, int a)
{
    // x-direction no-flux boundary: stays local to each GPU (every GPU owns the
    // full x-range). The y-direction boundary used to be applied here as well:
    //     if (j==jmin) data[psN(i,0,a)]    = data[psN(i,jmin,a)];   // periodic
    //     if (j==jmax) data[psN(i,Ny-1,a)] = data[psN(i,jmax,a)];   // periodic
    // In the multi-GPU version the y ghost rows are filled by halo_exchange()
    // (a ring copy between neighbouring GPUs), which also reproduces the global
    // periodic wrap for the top/bottom GPUs -- and reduces to exactly the old
    // periodic BC when only one GPU is used. So the y lines are intentionally
    // removed here.
    if (i==imin)      data[psN(0,    j,a)]  = data[psN(imin, j,a)];
    else if (i==imax) data[psN(Nx-1, j,a)] = data[psN(imax, j,a)];
}
                            ...
                            real *orientation, curandState *state, constants *para, long int step )
{
    ...
    if (i>0 && i<Nx-1 && j>=para->j_begin && j<=para->j_end)
    {
        ...
        if ( psi_next[ps]>1.0 )
        {
            ...
            for (a=0; a<num_orientation; a++)
            {
                phi_next[psN(i,j,a)] -= (psi_next[ps] - 1.0) * ( phi_next[psN(i,j,a)] > -1 )/num_phi_higher_than_minus1;
...
__global__ void compute_c( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next,
                            real *orientation, curandState *state, constants *para, long int step ) // flag
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    int a;
    ...
    if (i>0 && i<Nx-1 && j>=para->j_begin && j<=para->j_end)
    {
        ...
        real F10 = 0.25*(S10+S00+Shh+Shn)*( log(c[ps10]/c00)-bb*(g10-g00) );
        real F01 = 0.25*(S01+S00+Shh+Snh)*( log(c[ps01]/c00)-bb*(g01-g00) );
        real Fm0 = 0.25*(S00+Sm0+Snh+Snn)*( log(c[psm0]/c00)-bb*(gm0-g00) );
        real F0m = 0.25*(S00+S0m+Shn+Snn)*( log(c[ps0m]/c00)-bb*(g0m-g00) );

        real src10 = (F10+F01+Fm0+F0m)/(dx*dx);  // the divergence evaluated by <10>
        ...
        // real src11 = (F11+Fm1+F1m+Fmm)/(2.0*dx*dx);  // the divergence evaluated by <11>
...
__global__ void compute_c_CALPHAD( real *c, real *phi, real *psi, real *T_fine, real *c_next, real *phi_next, real *psi_next,  // flag
                                   real *orientation, curandState *state, constants *para, long int step, int i_offset_history, real l_offset_Vp )
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    if (i>0 && i<Nx-1 && j>=para->j_begin && j<=para->j_end)
    {
        ...
        real f00 = 0.5*(1.0+g00)*Gsc(T00, c00) + 0.5*(1.0-g00)*Glc(T00, c00);
        real f10 = 0.5*(1.0+g10)*Gsc(T10, c10) + 0.5*(1.0-g10)*Glc(T10, c10);
        real f01 = 0.5*(1.0+g01)*Gsc(T01, c01) + 0.5*(1.0-g01)*Glc(T01, c01);
        real fm0 = 0.5*(1.0+gm0)*Gsc(Tm0, cm0) + 0.5*(1.0-gm0)*Glc(Tm0, cm0);
        real f0m = 0.5*(1.0+g0m)*Gsc(T0m, c0m) + 0.5*(1.0-g0m)*Glc(T0m, c0m);
        ...
        real dc = para->dt*Dl/(Gamma*muk0) * (F10+F01+Fm0+F0m)/(dx*dx);
...
__global__ void pull_back( real *c, real *phi, real *psi, real *c_next, real *phi_next, real *psi_next, int i_offset, long int step, int j_begin, int j_end )
{
    ...
    // Only shift the rows this GPU owns. pull_back moves data along x within a
    // row, so it needs no y-neighbour data; the ghost rows are refreshed by the
    // halo exchange that runs before the next compute step.
    if (j<j_begin || j>j_end) return;
...
__global__ void integrate_phi(real *dphidt, real *psi, real *psi_next, constants *para, int i_left_end, long int step) // flag
{
    int iT = threadIdx.x + blockIdx.x * blockDim.x; // index of the temperature field
    int jT = threadIdx.y + blockIdx.y * blockDim.y;

    int NxT_all = para->NxT_all;
    int NyT = para->NyT;

    int iT1 = i_left_end/ratio_dxT_dx; // the first iT index for the left border of the PF simulation domain
    int iTn = (i_left_end+Nx-3)/ratio_dxT_dx; // the last iT index for the right border of the PF simulation domain

    real integral = 0;

    if ( iT>=iT1 && iT<=iTn && jT>=1 && jT<=NyT-2 )
    {
        int i0 = ratio_dxT_dx*iT + 1 - i_left_end; // in the T domain (iT1,jT), the first i index of the PF domain
        int in = i0 + ratio_dxT_dx - 1; // in the T domain (iT1,jT), the last i index of the PF domain
        if (i0<1) i0 = 1;
        if (in>Nx-2) in = Nx-2;
...
__global__ void compute_temperature(real *T, real *T_next, real *dphidt, constants *para, long int step, int sub_step)
{
    ...
    if ( iT>=1 && iT<=NxT-2 && jT>=1 && jT<=NyT-2 )
    {
        ...
        real dTdt = Vp_ramp(step*tau0*para->dt)*tau0/(SS*W0) * ( T10 - Tm0 )/(2.0*dxT) + laplacian*DT/(Gamma*muk0);
        T_next[ps00] = T[ps00] + para->dtT*dTdt + Lcp*dphidt[ps00]/( para->ratio_dt_dtT*pow2(ratio_dxT_dx) );
    }

    /////////////////// boundary condition
    if (iT==iTmin)      T_next[psT(0,     jT)] = para->Tcold;
    else if (iT==iTmax) T_next[psT(NxT-1, jT)] = para->Thot;
    if (jT==jTmin)      T_next[psT(iT, 0)]     = T_next[psT(iT, jTmin)];
    else if (jT==jTmax) T_next[psT(iT, NyT-1)] = T_next[psT(iT, jTmax)];
}
...
__global__ void interpolate_temperature(real *T, real *T_fine, constants *para, int i_left_end, long int step)
{
    ...
    if (i>=0 && i<Nx && j>=0 && j<Ny)
    {
        ...
        int iT = (i-1+i_left_end)/ratio_dxT_dx; // coarse grid; i=1 corresponds to i_left_end
        int jT = (j-1+dxT)/ratio_dxT_dx; // coarse grid; j=1 corresponds to dxT
...
__host__ __device__ real get_delta(constants *para, int i, int i_offset_history, real l_offset_Vp) // only for FTA
{
    return para->delta_initial 
            + GG*( i-i_tip_target + i_offset_history - para->i_offset_history_initial )*dx*SS*W0/deltaT0 
...
{
    char file_name[256];
    snprintf(file_name, sizeof(file_name), "%s/data/%s_%d.txt", path_input, field, index);
    ...
    else
    {
        for (int j=0; j<Ny; j=j+dj)
        {
            ...
            {
                if ( strcmp(field, "grain")==0 ) fprintf(fp, "%d\t", get_local_grain(data,i,j) );
                else fprintf(fp, "%g\t", data[pos(i,j)] );
            }
            fprintf(fp, "\n");
        }
        fclose(fp);
...
{
    char fname[256];
    snprintf(fname, sizeof(fname), "%s/data/check_point_%s.bin", path_input, field);
    int num_slices = 1;
    if ( strcmp(field, "phi")==0 )  num_slices = num_orientation;
...
void save_check_point_temperature( real *T, constants *para )
{
    int NxT_all = para->NxT_all;
    char fname[256];
    snprintf(fname, sizeof(fname), "%s/data/check_point_temperature.bin", path_input);
    ofstream fp(fname);
    if ( !fp )
    {
        cout<< fname << " cannot be saved!!"<<endl;
...
void set_parameters(constants *para)
{
    ...
    if constexpr (if_Vp_ramp) {
        para->step_pull_back = SS*W0*dx/(1.0*dt*tau0); // 103; // number of steps to pull back
    ...
    // printf("BLOCK_SIZE_X=%d, BLOCK_SIZE_Y=%d\n", Block[0], Block[1]);
    ...
    #if (ifTFC)
        real DDT = DT*tau0/pow2(SS*W0*ratio_dxT_dx); // dimensionless thermal diffusivity
        ...
        real length = 50.0; // um
        para->length_sample = ceil(length*1000/(dx*SS*W0*ratio_dxT_dx)) * (dx*SS*W0*ratio_dxT_dx); // nm
        para->NxT = para->length_sample/(dx*SS*W0*ratio_dxT_dx)+2; // "2" is for the boundary
        para->NxT_all = ceil(1.0*para->NxT/BLOCK_SIZE_T_X) * BLOCK_SIZE_T_X; // some lattices not used
        para->NyT = ((Ny-2)/ratio_dxT_dx)+2;  // "2" is for the boundary
        para->NyT_all = ceil(1.0*para->NyT/BLOCK_SIZE_T_Y) * BLOCK_SIZE_T_Y; // some lattices not used

        para->Tcold = TCOLD;
        para->Thot = para->Tcold + GG*para->length_sample;

        para->i_left_end_initial = (Tmelt - deltaT0 - para->Tcold)/(GG*dx*SS*W0) - i_tip_target;
...
void save_parameters( constants *para, int i_tip )
{
    ...
    fprintf(fp, "%-13g d0 [nm] = Gamma/deltaT0, capillary length, only for dilute alloys\n", d0);
    ...
    fprintf(fp, "%-13g eps1, anisotropy parameter of the interfacial free energy \n", eps1);
    fprintf(fp, "%-13g eps2, anisotropy parameter of the interfacial free energy \n", eps2*1.0);
    ...
    #if (ifTFC)
    fprintf(fp, "----------------------------------------- Variables for temperature field calculation (TFC)\n");
    fprintf(fp, "-------------------- Dynamically changing variables\n");
    fprintf(fp, "%-13d i_left_end_initial, The i index of the left border of PF in the T field.\n", para->i_left_end_initial);
    fprintf(fp, "%-13d i_left_end, The i index of the left border of PF in the T field.\n", para->i_left_end);
    fprintf(fp, "\n");
...
void load_parameters(constants *para)
{
    ...
    // 1. Skip the first header line "----------------... Dynamically changing variables:"
    fgets(dummy, sizeof(dummy), fp);

    // 2. Read Dynamically Changing Variables
    fscanf(fp, "%ld", &para->initial_step);              fgets(dummy, sizeof(dummy), fp);
    fscanf(fp, "%d", &para->i_offset_history_initial);  fgets(dummy, sizeof(dummy), fp);
    fscanf(fp, "%lf",&para->l_offset_Vp_initial);       fgets(dummy, sizeof(dummy), fp);
    fscanf(fp, "%lf",&para->delta_initial);             fgets(dummy, sizeof(dummy), fp);
    ...
    // 4. Read Grid/Physics Constants (Local vars in save function -> Read to temp)
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Nx_all
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Ny_all
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // dx
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // W0
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // S
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // W
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // i_tip_target

    // (Blank line is automatically skipped by fscanf whitespace handling)

    // 5. Read Time Variables
    fscanf(fp, "%lf", &para->dt);   fgets(dummy, sizeof(dummy), fp); // dt (maps to para field)
    fscanf(fp, "%lf", &temp);       fgets(dummy, sizeof(dummy), fp); // tau0
    fscanf(fp, "%ld", &temp_long);  fgets(dummy, sizeof(dummy), fp); // total_step
    fscanf(fp, "%d", &temp_int);    fgets(dummy, sizeof(dummy), fp); // step_fields
    fscanf(fp, "%d", &temp_int);    fgets(dummy, sizeof(dummy), fp); // step_check_point
    fscanf(fp, "%d", &temp_int);    fgets(dummy, sizeof(dummy), fp); // step_tip
    fscanf(fp, "%d", &temp_int);    fgets(dummy, sizeof(dummy), fp); // step_pull_back

    // 6. Read Material Properties 1
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Gamma
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // muk0
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Dl
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // GG
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Vp

    // 7. Read Material Properties 2
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // Tmelt
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // cinf
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // deltaT0
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // d0
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // ke
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // bb
    fscanf(fp, "%lf", &temp); fgets(dummy, sizeof(dummy), fp); // lambda

    // 8. Read Anisotropy / Random / Path
    fscanf(fp, "%lf", &temp);      fgets(dummy, sizeof(dummy), fp); // gamma0
    fscanf(fp, "%lf", &temp);      fgets(dummy, sizeof(dummy), fp); // gamma1
    fscanf(fp, "%lf", &temp);      fgets(dummy, sizeof(dummy), fp); // eps1
    fscanf(fp, "%lf", &temp);      fgets(dummy, sizeof(dummy), fp); // eps2
    fscanf(fp, "%lf", &temp);      fgets(dummy, sizeof(dummy), fp); // epk1
    fscanf(fp, "%ld", &temp_long); fgets(dummy, sizeof(dummy), fp); // random_seed
    fscanf(fp, "%s",  temp_str);   fgets(dummy, sizeof(dummy), fp); // path_input

    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // latentHeat
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // cp
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // Lcp
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // molar_weight
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // density
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // num_moles
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // latentHeat2
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // surface_tension
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // hh
    fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // h0

    // 9. Temperature Field Calculation (TFC) Block
    #if (ifTFC)
        fgets(dummy, sizeof(dummy), fp); // Blank
        fgets(dummy, sizeof(dummy), fp); // ----------------... Variables for TFC
        fgets(dummy, sizeof(dummy), fp); // -------------------- Dynamically changing variables

        fscanf(fp, "%d", &para->i_left_end_initial); fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%d", &para->i_left_end);         fgets(dummy, sizeof(dummy), fp);

        fscanf(fp, "%lf", &temp);             fgets(dummy, sizeof(dummy), fp); // dxT
        fscanf(fp, "%d", &temp_int);          fgets(dummy, sizeof(dummy), fp); // ratio_dxT_dx
        fscanf(fp, "%d", &para->NxT);         fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%d", &para->NyT);         fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%d", &para->NxT_all);     fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%d", &para->NyT_all);     fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%lf", &para->length_sample); fgets(dummy, sizeof(dummy), fp);

        // Time TFC
        fscanf(fp, "%lf", &para->dtT);        fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%d", &para->ratio_dt_dtT);fgets(dummy, sizeof(dummy), fp);

        // Thermal Props
        fscanf(fp, "%lf", &temp);    fgets(dummy, sizeof(dummy), fp); // DT
        fscanf(fp, "%lf", &para->Tcold); fgets(dummy, sizeof(dummy), fp);
        fscanf(fp, "%lf", &para->Thot);  fgets(dummy, sizeof(dummy), fp);
    #endif
...
void AutoBlockSize(int *Bloc, int devNx, int devNy)
{
    ...
    // Fallback logic: If we didn't find a perfect divisor (common with odd sizes),
    // reset to a standard efficient block size.
    // 32 is standard for X to match warp size (coalescing).
    if (best_metric == 100000) {
        Bloc[0] = 32;
        Bloc[1] = (BLOCKMAX / 32); 
        // Ensure we don't exceed devNy if devNy is very small
        if (Bloc[1] > devNy) Bloc[1] = devNy; 
    }
}
...
void DisplayDeviceProperties(int index)
{
    ...
    if( cudaSuccess == cudaGetDeviceProperties(&deviceProp, index))
    {
        ...
        printf( "\n--------------------------------------------------------------------------------");
        printf( "\nTotal Global Memory                  \t %g GB", deviceProp.totalGlobalMem/pow3(1024.0) );
        ...
        printf( "\nMemory Pitch                     \t %g GB", deviceProp.memPitch/pow3(1024.0) );
        ...
        printf( "\nTotal constant memory            \t %g KB", deviceProp.totalConstMem/1024.0 );
        printf( "\nCUDA ver                         \t %d.%d", deviceProp.major, deviceProp.minor );
        int rate;
        if ( cudaSuccess == cudaDeviceGetAttribute(&rate, cudaDevAttrClockRate, index) ) {
            printf( "\nClock rate                       \t %g GHz", rate/pow2(1000.0) );
        }
        else {
            printf( "\nClock rate                       \t Not found" );
        }
        // printf( "\nClock rate                       \t %g GHz", deviceProp.clockRate/pow2(1000.0) );
        printf( "\nTexture Alignment                \t %ld bytes", (long int)(deviceProp.textureAlignment) );
        printf( "\nDevice Overlap                   \t %s", deviceProp.asyncEngineCount?"Allowed":"Not Allowed" );
        printf( "\nNumber of Multi processors       \t %d", deviceProp.multiProcessorCount );
        printf( "\n--------------------------------------------------------------------------------");
        printf( "\nNumber of registers per thread \t\t %d", deviceProp.regsPerBlock/(BLOCK_SIZE_X*BLOCK_SIZE_Y) );
        printf( "\nNumber of real varibles per thread \t %d", deviceProp.regsPerBlock/(BLOCK_SIZE_X*BLOCK_SIZE_Y)/2 );
        printf( "\nNumber of int varibles per thread \t %d", deviceProp.regsPerBlock/(BLOCK_SIZE_X*BLOCK_SIZE_Y) );
        printf( "\n================================================================================");
        printf( "\n\n" );
    }
    else
...
// Multi-GPU version of set_cuda_device(): use up to MAX_GPU devices at once.
// Unlike the single-GPU path, this claims every visible device (ordinals
// 0..nGPU-1) rather than searching for a single "free" one -- a multi-GPU run
// is expected to have the node to itself. Peer (P2P) access is enabled between
// ring neighbours so the halo exchange can copy device-to-device; if a pair
// cannot peer, cudaMemcpyPeer still works (it stages through the host), so any
// failure here is non-fatal.
int set_cuda_devices(MultiGPU *mg)
{
    int Ndevices = 0;
    ...
    mg->nGPU = Ndevices < MAX_GPU ? Ndevices : MAX_GPU;
    printf("Using %d GPU(s) (of %d visible).\n", mg->nGPU, Ndevices);

    for (int g=0; g<mg->nGPU; g++)
    {
        mg->dev[g] = g;
        DisplayDeviceProperties(g);
    }

    // Enable P2P between ring neighbours (best effort).
    if (mg->nGPU > 1)
    {
        for (int g=0; g<mg->nGPU; g++)
        {
            int up = (g+1)%mg->nGPU;
            int dn = (g-1+mg->nGPU)%mg->nGPU;
            cudaSetDevice(mg->dev[g]);
            int can=0;
            cudaDeviceCanAccessPeer(&can, mg->dev[g], mg->dev[up]);
            if (can) { cudaDeviceEnablePeerAccess(mg->dev[up], 0); cudaGetLastError(); }
            if (dn != up)
            {
                cudaDeviceCanAccessPeer(&can, mg->dev[g], mg->dev[dn]);
                if (can) { cudaDeviceEnablePeerAccess(mg->dev[dn], 0); cudaGetLastError(); }
            }
        }
    }
...
static inline int deal_with_boundary(int x, int N0, int Nn)
{
    ...
    int Nmax = max(N0, Nn);
    #if (Periodic)
        if (x <= Nmin-1)      x += (Nmax-Nmin+1);
        else if (x >= Nmax+1) x -= (Nmax-Nmin+1);
    #elif (NoFlux)
        if (x < Nmin)          x = 2*Nmin-x;
        else if (x > Nmax)     x = 2*Nmax-x;
    #endif
...
// Helper: Solves A * c = b for a 5x5 system using Gaussian Elimination with partial pivoting
...
static int solve_3x3(real A[3][3], real cc[3], real x[3])
{
    for (int k = 0; k < 3; ++k) {
        ...
        if (maxval < 1e-14) return -1; // singular
                    ...
                    real *R)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        M[0][0] += f0*f0;  M[0][1] += f0*f1;  M[0][2] += f0*f2;
        M[1][0] += f1*f0;  M[1][1] += f1*f1;  M[1][2] += f1*f2;
        M[2][0] += f2*f0;  M[2][1] += f2*f1;  M[2][2] += f2*f2;
...
int get_tip_int_at_j( real *psi, int i_initial, int j )
{
    ...
    {
// if ( (psi[pos(i,j)]-phi_interface)*(psi[pos(i+1,j)]-phi_interface)<0 ||
// abs(psi[pos(i,j)]-phi_interface)<1.0e-6 )
	if (psi[pos(i,j)] > phi_interface)
        {
            itip = i;
        }
...
void get_tip_int( real *phi, ij *tip )
{
    ...
    for (int j=0; j<Ny; j++)
    {
        ...
        real iphi0 = (phi[pos(i0+1,j)]-phi[pos(i0,j)])*(phi_interface-phi[pos(i0,j)]) + i0;
        if (iphi0>itip)
        {
            itip=iphi0;
            tip->i = int(itip);
...
void get_precise_tip(real *psi, TIP *tip, long int step)
{
    ...
    if (tip->R > 1e5) tip->R=1e5;
    if (tip->R < 1.0) tip->R=1.0;
                    ...
                    real start_time, long int step, dim3 numBlocks, dim3 threadsPerBlock)
{
    if ( step%h_para->step_tip==0 || step==h_para->initial_step) // flag
    {
        ...
        real V_tip = (x_tip_now- h_para->x_tip_history)*SS*W0/(h_para->step_tip*tau0*dt); // m/s
        h_para->x_tip_history = x_tip_now;
        if (V_tip<0.0) V_tip = 0.01;
        if (V_tip>10.0)V_tip = 10.0;

        #if (ifTFC)
            // T_fine is replicated on every GPU; read it from GPU 0.
            cudaSetDevice(mg->dev[0]);
            interpolate_temperature<<<numBlocks, threadsPerBlock>>>(mg->d_T[0], mg->d_T_fine[0], mg->d_para[0], h_para->i_left_end, step);
            cudaMemcpy(h_T_fine, mg->d_T_fine[0], size_grid, cudaMemcpyDeviceToHost);

            real T_tip = h_T_fine[pos(tip.i, 0)];
            real delta = (T_tip - Tmelt)/deltaT0;
        #else
            real delta = get_delta(h_para, tip.i, h_para->i_offset_history, h_para->l_offset_Vp);
            real T_tip = delta*deltaT0 + Tmelt;
        #endif
        h_para->delta = delta; // for save_parameters()

        // printf("step=%d, delta=%g, i_offset_history=%d, l_offset_Vp=%g, i_left_end=%d, i_left_end_initial=%d\n", 
        //     step, delta, h_para->i_offset_history, h_para->l_offset_Vp, h_para->i_left_end, h_para->i_left_end_initial);
